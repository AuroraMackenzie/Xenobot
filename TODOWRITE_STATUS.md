...
- 7.2 12个AI工具: 进行中(已新增AI语义检索端点`POST /ai/semantic-search-messages`，支持分块嵌入+余弦相似度阈值检索)
- 7.3 集成LLM（OpenAI兼容、Gemini、DeepSeek、硅基流动(Silicon Flow)）: 完成(简化)
- 7.4 API密钥加密存储: 完成(简化)

## 8. RAG和语义搜索（GPU加速）
- 8.1 文本分块: 完成(简化)(CLI语义检索已接入分块器：固定窗口+重叠窗口，避免长文本单向量丢失上下文)
...
- 12.3 数据查询和导出命令: 完成(简化)(已支持query: search/sql/semantic；其中semantic路径已从关键词回退升级为分块向量相似度检索；export: jsonl/text/csv/json/html 基础导出)
...
- 16.2 API文档: 进行中(已新增`docs/API.md`基线文档，覆盖当前Axum路由清单、`/import-batch`两种模式语义、SSE与增量导入接口；仍需补充字段级请求/响应示例与错误码规范)
- 16.3 用户指南和教程: 未完成
