//! Media helpers for legal-safe Discord export workflows.

use std::fs;
use std::path::{Path, PathBuf};

/// Media kind inferred from a Discord export asset path.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum DiscordMediaKind {
    /// Image asset.
    Image,
    /// Video asset.
    Video,
...
pub struct DiscordMediaAsset {
    /// Original asset path.
    pub path: PathBuf,
    /// Inferred media kind.
    pub kind: DiscordMediaKind,
...
pub fn classify_media_path(path: &Path) -> DiscordMediaKind {
    let ext = path
        .extension()
    ...
    match ext.as_deref() {
        Some("jpg" | "jpeg" | "png" | "gif" | "bmp" | "heic" | "webp") => DiscordMediaKind::Image,
        Some("mp4" | "mov" | "avi" | "mkv" | "webm") => DiscordMediaKind::Video,
        Some("ogg" | "opus" | "mp3" | "m4a" | "wav" | "aac" | "flac") => DiscordMediaKind::Audio,
        Some("pdf" | "doc" | "docx" | "xls" | "xlsx" | "ppt" | "pptx" | "txt" | "csv") => {
            DiscordMediaKind::Document
        }
        Some("zip" | "7z" | "rar" | "tar" | "gz") => DiscordMediaKind::Archive,
        Some("svg") => DiscordMediaKind::Sticker,
        _ => DiscordMediaKind::Unknown,
    }
}

/// Build a media inventory from explicitly provided asset paths.
pub fn collect_media_assets<I, P>(paths: I) -> Vec<DiscordMediaAsset>
where
...
mod tests {
    ...
    fn collects_existing_media_assets() {
        ...
        assert_eq!(assets[0].kind, DiscordMediaKind::Image);
