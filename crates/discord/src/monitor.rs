//! File monitoring for legal-safe Discord export directories.
...
/// File system event types emitted by the Discord monitor.
...
/// File monitoring configuration for Discord exports.
#[derive(Debug, Clone)]
...
/// File system monitor for Discord export trees.
...
impl FileMonitor {
    ...
    pub fn new(config: FileMonitorConfig) -> Result<Self, DiscordError> {
    ...
    /// Default path patterns for Discord export and media folders.
    pub fn discord_export_patterns() -> Vec<Regex> {
        vec![
...
mod tests {
    ...
    fn export_patterns_match_common_assets() {
        let patterns = FileMonitor::discord_export_patterns();
        let export_path = Path::new("/tmp/Export/channel.json");
