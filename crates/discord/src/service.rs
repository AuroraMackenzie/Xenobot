//! Service layer for legal-safe Discord ingestion workflows.
...
use crate::media::{collect_media_assets, DiscordMediaAsset};
use crate::monitor::{FileMonitor, FileMonitorConfig};
use crate::{DiscordAdapter, DiscordError};

/// Parsed export staged by the Discord service.
#[derive(Debug, Clone)]
pub struct StagedDiscordExport {
...
/// Aggregated legal-safe workspace assembled from explicit Discord inputs.
#[derive(Debug, Clone)]
pub struct AuthorizedDiscordWorkspace {
    /// Stable platform identifier.
    pub platform_id: &'static str,
...
/// Legal-safe Discord orchestration service.
...
impl DiscordService {
    ...
    /// Return all discovered account views.
    ...
    pub fn parse_authorized_export(&self, path: &Path) -> Result<ParsedChat, DiscordError> {
    ...
    pub fn stage_authorized_exports<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<StagedDiscordExport>, DiscordError>
    where
    ...
    pub fn collect_media_inventory<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<DiscordMediaAsset>, DiscordError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
    {
        let resolved: Result<Vec<PathBuf>, DiscordError> = paths
            .into_iter()
            .map(|path| {
    ...
    pub fn build_authorized_workspace<I, P, J, Q>(
        ...
        export_paths: I,
        media_paths: J,
    ) -> Result<AuthorizedDiscordWorkspace, DiscordError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
    ...
    ) -> Result<(AuthorizedDiscordWorkspace, Option<FileMonitor>), DiscordError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        J: IntoIterator<Item = Q>,
    ...
    pub fn create_export_monitor(&self, watch_dir: &Path) -> Result<FileMonitor, DiscordError> {
    ...
    fn ensure_authorized(&self, path: &Path) -> Result<(), DiscordError> {
...
mod tests {
    ...
    fn returns_platform_id_from_service() {
        let service = DiscordService::new(DiscordConfig::default());
        assert_eq!(service.platform_id(), "discord");
    }
    ...
    fn rejects_paths_outside_authorized_roots() {
        ...
        match err {
            DiscordError::UnauthorizedPath { path } => {
    ...
    fn build_authorized_workspace_rejects_unauthorized_media_paths() {
        ...
        let export = export_dir.path().join("discord_fixture.json");
    ...
    fn builds_authorized_workspace_from_exports_and_media() {
        ...
        assert_eq!(workspace.platform_id, "discord");
    ...
    fn prepare_authorized_workspace_rejects_unauthorized_watch_directory() {
        ...
        let export = input_dir.path().join("discord_fixture.json");
        write_fixture(
            &export,
            r#"[{"ID":"1","Timestamp":"2025-01-02T10:20:30Z","Author":{"ID":"u1","Name":"Alice"},"Content":"hello discord"}]"#,
        );
    ...
    fn authorized_workspace_reports_empty_when_no_exports_or_media_are_staged() {
        let service =
            DiscordService::new(DiscordConfig::with_authorized_roots([std::env::temp_dir()]));
        let workspace = service
    ...
    fn collect_media_inventory_rejects_unauthorized_assets() {
        ...
        match error {
            DiscordError::UnauthorizedPath { path } => assert_eq!(path, asset),
            other => panic!("unexpected error: {other:?}"),
        }
    }
