//! Audio transcoding helpers for legal-safe Discord media workflows.
//!
...
use crate::{DiscordError, DiscordResult};
use std::io::Write;
...
pub fn transcode_audio_to_mp3(
    ...
    if !input_path.exists() {
        return Err(DiscordError::Io(std::io::Error::new(
    ...
    if let Some(parent) = output_path.parent() {
        std::fs::create_dir_all(parent).map_err(DiscordError::Io)?;
    }
    ...
    let output = Command::new(binary)
        ...
        .output()
        .map_err(|error| {
            DiscordError::Internal(anyhow::anyhow!("failed to start ffmpeg: {}", error))
        })?;
    ...
    Err(DiscordError::Internal(anyhow::anyhow!(
...
pub fn transcode_audio_bytes_to_mp3(
    ...
    input_format: &str,
    options: &AudioTranscodeOptions,
) -> DiscordResult<Vec<u8>> {
    if input_bytes.is_empty() {
        return Err(DiscordError::Internal(anyhow::anyhow!(
    ...
    if let Some(mut stdin) = child.stdin.take() {
        stdin.write_all(input_bytes).map_err(|error| {
            DiscordError::Internal(anyhow::anyhow!("failed to write ffmpeg stdin: {}", error))
        })?;
    }

    let output = child.wait_with_output().map_err(|error| {
        DiscordError::Internal(anyhow::anyhow!("failed to wait ffmpeg: {}", error))
    })?;
...
mod tests {
    ...
    fn missing_input_file_returns_not_found() {
        let options = AudioTranscodeOptions::default();
        let input_path = Path::new("/tmp/discord-missing-input.ogg");
        let output_path = Path::new("/tmp/discord-missing-output.mp3");
        let error = transcode_audio_to_mp3(input_path, output_path, &options)
            .expect_err("missing input should fail");

        match error {
            DiscordError::Io(inner) => assert_eq!(inner.kind(), std::io::ErrorKind::NotFound),
