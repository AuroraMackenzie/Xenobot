//! Account models for legal-safe Discord contexts.
//!
...
/// Represents a discovered Discord account context.
...
mod tests {
    ...
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/discord-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "discord-export");
    }

    #[test]
    fn root_path_reflects_data_dir() {
        let candidate = source(SourceKind::ExportDirectory, "Archive", "/tmp/discord-root");
        let account = Account::from_source(&candidate);
