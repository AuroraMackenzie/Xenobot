//! Legal-safe Discord adapter for Xenobot.
//!
...
pub const PLATFORM_ID: &str = "discord";
/// Common result type used by Discord crate operations.
pub type DiscordResult<T> = Result<T, DiscordError>;

pub use config::DiscordConfig;
pub use service::{AuthorizedDiscordWorkspace, DiscordService, StagedDiscordExport};

/// Legal-safe adapter for Discord workflows.
...
impl DiscordAdapter {
    ...
    pub fn parse_authorized_export(&self, path: &Path) -> Result<ParsedChat, DiscordError> {
        ...
        let parsed = registry
            ...
            .map_err(DiscordError::Parse)?;

        if parsed.platform.eq_ignore_ascii_case(PLATFORM_ID) {
            Ok(parsed)
        } else {
            Err(DiscordError::PlatformMismatch {
