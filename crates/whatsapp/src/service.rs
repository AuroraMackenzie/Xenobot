//! Service layer for legal-safe WhatsApp ingestion workflows.
...
use crate::media::{collect_media_assets, WhatsAppMediaAsset};
use crate::monitor::{FileMonitor, FileMonitorConfig};
use crate::{WhatsAppAdapter, WhatsAppError};

/// Parsed export staged by the WhatsApp service.
#[derive(Debug, Clone)]
pub struct StagedWhatsAppExport {
...
/// Aggregated legal-safe workspace assembled from explicit WhatsApp inputs.
#[derive(Debug, Clone)]
pub struct AuthorizedWhatsAppWorkspace {
    /// Stable platform identifier.
    pub platform_id: &'static str,
...
/// Legal-safe WhatsApp orchestration service.
...
impl WhatsAppService {
    ...
    pub fn stage_authorized_exports<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<StagedWhatsAppExport>, WhatsAppError>
    where
    ...
    pub fn collect_media_inventory<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<WhatsAppMediaAsset>, WhatsAppError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
    {
        let resolved: Result<Vec<PathBuf>, WhatsAppError> = paths
            .into_iter()
            .map(|path| {
    ...
    pub fn build_authorized_workspace<I, P, J, Q>(
        ...
        export_paths: I,
        media_paths: J,
    ) -> Result<AuthorizedWhatsAppWorkspace, WhatsAppError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        ...
        Q: AsRef<Path>,
    {
        ...
        Ok(AuthorizedWhatsAppWorkspace {
            platform_id: self.platform_id(),
            accounts,
            primary_account,
            staged_exports,
    ...
    ) -> Result<(AuthorizedWhatsAppWorkspace, Option<FileMonitor>), WhatsAppError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        J: IntoIterator<Item = Q>,
    ...
    pub fn create_export_monitor(&self, watch_dir: &Path) -> Result<FileMonitor, WhatsAppError> {
        self.ensure_authorized(watch_dir)?;
        FileMonitor::new(FileMonitorConfig {
            watch_dir: watch_dir.to_path_buf(),
            file_patterns: FileMonitor::whatsapp_export_patterns(),
            recursive: true,
        })
    }

    fn ensure_authorized(&self, path: &Path) -> Result<(), WhatsAppError> {
...
mod tests {
    ...
    fn returns_platform_id_from_service() {
        let service = WhatsAppService::new(WhatsAppConfig::default());
        assert_eq!(service.platform_id(), "whatsapp");
    }
    ...
    fn rejects_paths_outside_authorized_roots() {
        ...
        match err {
            WhatsAppError::UnauthorizedPath { path } => {
    ...
    fn build_authorized_workspace_rejects_unauthorized_media_paths() {
        ...
        let export = export_dir.path().join("whatsapp_fixture.txt");
    ...
    fn builds_authorized_workspace_from_exports_and_media() {
        ...
        assert_eq!(workspace.platform_id, "whatsapp");
    ...
    fn collect_media_inventory_rejects_unauthorized_assets() {
        ...
        match error {
            WhatsAppError::UnauthorizedPath { path } => assert_eq!(path, asset),
            other => panic!("unexpected error: {other:?}"),
        }
    }
