//! Account models for legal-safe WhatsApp contexts.
//!
...
/// Represents a discovered WhatsApp account context.
...
mod tests {
    ...
    fn builds_account_from_source() {
        let candidate = source(
            ...
            "macOS WhatsApp sandbox data",
            "/Users/test/Library/Containers/WhatsApp",
        );
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "macOS WhatsApp sandbox data");
    ...
    fn deduplicates_accounts_by_root_path() {
        let candidates = vec![
            source(SourceKind::ExportDirectory, "Downloads export", "/tmp/wa"),
            source(SourceKind::AppContainer, "Sandbox", "/tmp/wa"),
        ];
    ...
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/whatsapp-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "whatsapp-export");
    }

    #[test]
    fn root_path_reflects_data_dir() {
        let candidate = source(SourceKind::ExportDirectory, "Archive", "/tmp/whatsapp-root");
        let account = Account::from_source(&candidate);

        assert_eq!(account.root_path(), Path::new("/tmp/whatsapp-root"));
    }
