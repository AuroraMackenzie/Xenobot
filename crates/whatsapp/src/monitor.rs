//! File monitoring for legal-safe WhatsApp export directories.
...
/// File system event types emitted by the WhatsApp monitor.
...
/// File monitoring configuration for WhatsApp exports.
#[derive(Debug, Clone)]
...
/// File system monitor for WhatsApp export trees.
...
impl FileMonitor {
    ...
    pub fn new(config: FileMonitorConfig) -> Result<Self, WhatsAppError> {
    ...
    /// Start monitoring.
    pub fn start(&mut self) -> Result<(), WhatsAppError> {
    ...
    /// Stop monitoring.
    pub fn stop(&mut self) -> Result<(), WhatsAppError> {
    ...
    /// Default path patterns for WhatsApp export/media folders.
    pub fn whatsapp_export_patterns() -> Vec<Regex> {
        vec![
            Regex::new(r"(?i)\.(txt|zip|json|csv)$").expect("valid export regex"),
            Regex::new(r"(?i)\.(jpg|jpeg|png|webp|gif|mp4|mov|opus|ogg|pdf)$")
...
mod tests {
    ...
    fn export_patterns_match_common_assets() {
        let patterns = FileMonitor::whatsapp_export_patterns();
        let export_path = Path::new("/tmp/Export/chat.zip");
        let image_path = Path::new("/tmp/Media/photo.jpg");
