//! Audio transcoding helpers for legal-safe WhatsApp media workflows.
//!
...
use crate::{WhatsAppError, WhatsAppResult};
use std::io::Write;
...
pub fn transcode_audio_to_mp3(
    ...
    if !input_path.exists() {
        return Err(WhatsAppError::Io(std::io::Error::new(
    ...
    if let Some(parent) = output_path.parent() {
        std::fs::create_dir_all(parent).map_err(WhatsAppError::Io)?;
    }
    ...
    let output = Command::new(binary)
        ...
        .output()
        .map_err(|error| {
            WhatsAppError::Internal(anyhow::anyhow!("failed to start ffmpeg: {}", error))
        })?;
    ...
    Err(WhatsAppError::Internal(anyhow::anyhow!(
...
pub fn transcode_audio_bytes_to_mp3(
    ...
    if input_bytes.is_empty() {
        return Err(WhatsAppError::Internal(anyhow::anyhow!(
    ...
    if let Some(mut stdin) = child.stdin.take() {
        stdin.write_all(input_bytes).map_err(|error| {
            WhatsAppError::Internal(anyhow::anyhow!("failed to write ffmpeg stdin: {}", error))
        })?;
    }

    let output = child.wait_with_output().map_err(|error| {
        WhatsAppError::Internal(anyhow::anyhow!("failed to wait ffmpeg: {}", error))
    })?;
