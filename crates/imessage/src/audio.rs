...
use crate::{IMessageError, IMessageResult};
use std::io::Write;
...
pub fn transcode_audio_to_mp3(
    ...
    if let Some(parent) = output_path.parent() {
        std::fs::create_dir_all(parent).map_err(IMessageError::Io)?;
    }
...
pub fn transcode_audio_bytes_to_mp3(
    ...
    if let Some(mut stdin) = child.stdin.take() {
        stdin.write_all(input_bytes).map_err(|error| {
            IMessageError::Internal(anyhow::anyhow!("failed to write ffmpeg stdin: {}", error))
        })?;
    }

    let output = child.wait_with_output().map_err(|error| {
        IMessageError::Internal(anyhow::anyhow!("failed to wait ffmpeg: {}", error))
    })?;
...
mod tests {
    ...
    fn missing_input_file_returns_not_found() {
        let options = AudioTranscodeOptions::default();
        let input_path = Path::new("/tmp/imessage-missing-input.ogg");
        let output_path = Path::new("/tmp/imessage-missing-output.mp3");
        let error = transcode_audio_to_mp3(input_path, output_path, &options)
            .expect_err("missing input should fail");

        match error {
            IMessageError::Io(inner) => assert_eq!(inner.kind(), std::io::ErrorKind::NotFound),
