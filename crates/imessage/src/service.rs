...
mod tests {
    ...
    fn rejects_paths_outside_authorized_roots() {
        let service = IMessageService::new(IMessageConfig::with_authorized_roots([PathBuf::from(
            "/tmp/allowed",
        )]));
    ...
    fn builds_media_inventory_for_authorized_assets() {
        ...
        let service = IMessageService::new(IMessageConfig::with_authorized_roots([dir
            .path()
            .to_path_buf()]));
    ...
    fn build_authorized_workspace_rejects_unauthorized_media_paths() {
        ...
        let service = IMessageService::new(IMessageConfig::with_authorized_roots([export_dir
            .path()
            .to_path_buf()]));
    ...
    fn build_authorized_workspace_rejects_unauthorized_export_paths() {
        ...
        let export = unauthorized_dir
            .path()
            .join("imessage_unauthorized_fixture.dat");
    ...
    fn prepare_authorized_workspace_rejects_unauthorized_watch_directory() {
        ...
        let service = IMessageService::new(IMessageConfig::with_authorized_roots([input_dir
            .path()
            .to_path_buf()]));
    ...
    fn authorized_workspace_reports_empty_when_no_exports_or_media_are_staged() {
        let service =
            IMessageService::new(IMessageConfig::with_authorized_roots(
                [std::env::temp_dir()],
            ));
        let workspace = service
    ...
    fn rejects_audio_asset_transcoding_when_input_path_is_not_authorized() {
        ...
        let service = IMessageService::new(IMessageConfig::with_authorized_roots([output_dir
            .path()
            .to_path_buf()]));
        let error = service
            .transcode_audio_asset_to_mp3(&input, &output, &AudioTranscodeOptions::default())
    ...
    fn add_authorized_root_allows_runtime_monitor_creation() {
        ...
        let mut service = IMessageService::new(IMessageConfig::with_authorized_roots([other_dir
            .path()
            .to_path_buf()]));
    ...
    fn add_authorized_root_allows_runtime_audio_output_validation_to_progress() {
        ...
        let mut service = IMessageService::new(IMessageConfig::with_authorized_roots([input_dir
            .path()
            .to_path_buf()]));
        assert!(matches!(
    ...
    fn stage_authorized_exports_rejects_unauthorized_path_directly() {
        ...
        let service = IMessageService::new(IMessageConfig::with_authorized_roots([authorized_dir
            .path()
            .to_path_buf()]));
