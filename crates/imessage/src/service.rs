//! Service layer for legal-safe IMessage ingestion workflows.
...
use crate::media::{collect_media_assets, IMessageMediaAsset};
use crate::monitor::{FileMonitor, FileMonitorConfig};
use crate::{IMessageAdapter, IMessageError};

/// Parsed export staged by the IMessage service.
#[derive(Debug, Clone)]
pub struct StagedIMessageExport {
...
/// Aggregated legal-safe workspace assembled from explicit iMessage inputs.
#[derive(Debug, Clone)]
pub struct AuthorizedIMessageWorkspace {
    /// Stable platform identifier.
    pub platform_id: &'static str,
...
/// Legal-safe IMessage orchestration service.
...
impl IMessageService {
    ...
    pub fn stage_authorized_exports<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<StagedIMessageExport>, IMessageError>
    where
    ...
    pub fn collect_media_inventory<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<IMessageMediaAsset>, IMessageError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
    {
        let resolved: Result<Vec<PathBuf>, IMessageError> = paths
            .into_iter()
            .map(|path| {
        ...
        Ok(collect_media_assets(resolved?))
    }
    ...
    pub fn build_authorized_workspace<I, P, J, Q>(
        ...
        export_paths: I,
        media_paths: J,
    ) -> Result<AuthorizedIMessageWorkspace, IMessageError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        ...
        Q: AsRef<Path>,
    {
        ...
        Ok(AuthorizedIMessageWorkspace {
            platform_id: self.platform_id(),
            accounts,
            primary_account,
            staged_exports,
    ...
    ) -> Result<(AuthorizedIMessageWorkspace, Option<FileMonitor>), IMessageError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        J: IntoIterator<Item = Q>,
    ...
    pub fn transcode_audio_payload_to_mp3(
        ...
        input_format: &str,
        options: &AudioTranscodeOptions,
    ) -> Result<Vec<u8>, IMessageError> {
        transcode_audio_bytes_to_mp3(input_bytes, input_format, options)
    }

    /// Create a file monitor for an explicitly authorized export directory.
    pub fn create_export_monitor(&self, watch_dir: &Path) -> Result<FileMonitor, IMessageError> {
        self.ensure_authorized(watch_dir)?;
        FileMonitor::new(FileMonitorConfig {
            watch_dir: watch_dir.to_path_buf(),
            file_patterns: FileMonitor::imessage_export_patterns(),
            recursive: true,
        })
    }

    fn ensure_authorized(&self, path: &Path) -> Result<(), IMessageError> {
        if self.config.is_authorized_path(path) {
            Ok(())
        } else {
            Err(IMessageError::UnauthorizedPath {
...
mod tests {
    ...
    fn returns_platform_id_from_service() {
        let service = IMessageService::new(IMessageConfig::default());
        assert_eq!(service.platform_id(), "imessage");
    }

    #[test]
    fn rejects_paths_outside_authorized_roots() {
        let service = IMessageService::new(IMessageConfig::with_authorized_roots([PathBuf::from("/tmp/allowed")]));
        ...
        match err {
            IMessageError::UnauthorizedPath { path } => {
    ...
    fn builds_media_inventory_for_authorized_assets() {
        ...
        let service = IMessageService::new(IMessageConfig::with_authorized_roots([dir.path().to_path_buf()]));

        let assets = service
            .collect_media_inventory([asset.as_path()])
            .expect("authorized asset should be indexed");
    ...
    fn build_authorized_workspace_rejects_unauthorized_media_paths() {
        ...
        let export = export_dir.path().join("imessage_fixture.json");
        let media = media_dir.path().join("preview.heic");
        write_fixture(&export, r#"[{"text":"hello imessage","sender":"Alice","date":"2025-01-02T10:20:30Z"}]"#);
        std::fs::write(&media, [1_u8, 2, 3]).expect("media fixture");

        let service =
            IMessageService::new(IMessageConfig::with_authorized_roots([export_dir.path().to_path_buf()]));

        match service.build_authorized_workspace([export.as_path()], [media.as_path()]) {
            Err(IMessageError::UnauthorizedPath { path }) => assert_eq!(path, media),
    ...
    fn build_authorized_workspace_rejects_unauthorized_export_paths() {
        ...
        let export = unauthorized_dir.path().join("imessage_unauthorized_fixture.dat");
        ...
        match service.build_authorized_workspace([export.as_path()], std::iter::empty::<&Path>()) {
            Err(IMessageError::UnauthorizedPath { path }) => assert_eq!(path, export),
    ...
    fn builds_authorized_workspace_from_exports_and_media() {
        let dir = tempdir().expect("tempdir");
        let export = dir.path().join("imessage_fixture.json");
        let asset = dir.path().join("voice.ogg");
        ...
        let service =
            IMessageService::new(IMessageConfig::with_authorized_roots([dir.path().to_path_buf()]));
        let workspace = service
            .build_authorized_workspace([export.as_path()], [asset.as_path()])
            .expect("workspace should build");

        assert_eq!(workspace.platform_id, "imessage");
    ...
    fn prepare_authorized_workspace_rejects_unauthorized_watch_directory() {
        ...
        let export = input_dir.path().join("imessage_fixture.json");
        write_fixture(
            &export,
            r#"[{"text":"hello imessage","sender":"Alice","date":"2025-01-02T10:20:30Z"}]"#,
        );
        ...
        let service =
            IMessageService::new(IMessageConfig::with_authorized_roots([input_dir.path().to_path_buf()]));
        match service.prepare_authorized_workspace(
            [export.as_path()],
            std::iter::empty::<&Path>(),
            Some(watch_dir.path()),
        ) {
            Err(IMessageError::UnauthorizedPath { path }) => {
    ...
    fn authorized_workspace_reports_empty_when_no_exports_or_media_are_staged() {
        let service = IMessageService::new(IMessageConfig::with_authorized_roots([std::env::temp_dir()]));
        let workspace = service
    ...
    fn rejects_audio_asset_transcoding_when_output_directory_is_not_authorized() {
        ...
        let service = IMessageService::new(IMessageConfig::with_authorized_roots([input_dir.path().to_path_buf()]));
        let error = service
    ...
    fn rejects_audio_asset_transcoding_when_input_path_is_not_authorized() {
        let input_dir = tempdir().expect("tempdir");
        let output_dir = tempdir().expect("tempdir");
        let input = input_dir.path().join("voice.m4a");
        let output = output_dir.path().join("voice.mp3");
        fs::write(&input, [1_u8, 2, 3]).expect("write input");

        let service =
            IMessageService::new(IMessageConfig::with_authorized_roots([output_dir.path().to_path_buf()]));
        let error = service
            .transcode_audio_asset_to_mp3(&input, &output, &AudioTranscodeOptions::default())
    ...
    fn add_authorized_root_allows_runtime_monitor_creation() {
        ...
        let mut service =
            IMessageService::new(IMessageConfig::with_authorized_roots([other_dir.path().to_path_buf()]));
    ...
    fn collect_media_inventory_rejects_unauthorized_assets() {
        let media_dir = tempdir().expect("tempdir");
        let other_dir = tempdir().expect("tempdir");
        let asset = media_dir.path().join("preview.heic");
        std::fs::write(&asset, [1_u8, 2, 3]).expect("media fixture");

        let mut service =
            IMessageService::new(IMessageConfig::with_authorized_roots([other_dir.path().to_path_buf()]));
        match service.collect_media_inventory([asset.as_path()]) {
            Err(IMessageError::UnauthorizedPath { path }) => assert_eq!(path, asset),
    ...
    fn add_authorized_root_allows_runtime_workspace_preparation_with_monitor() {
        ...
        let other_dir = tempdir().expect("tempdir");
        let export = dir.path().join("imessage_fixture.json");
        std::fs::write(&export, r#"[{"text":"hello imessage","sender":"Alice","date":"2025-01-02T10:20:30Z"}]"#).expect("fixture");
    ...
    fn add_authorized_root_allows_runtime_audio_input_validation_to_progress() {
        ...
        match service.transcode_audio_asset_to_mp3(&input, &output, &AudioTranscodeOptions::default()) {
            Err(IMessageError::UnauthorizedPath { path }) => assert_eq!(path, input),
            _ => panic!("expected unauthorized path before runtime authorization"),
        }

        service.add_authorized_root(input_dir.path().to_path_buf());
        if let Err(IMessageError::UnauthorizedPath { path }) =
    ...
    fn add_authorized_root_allows_runtime_export_parsing() {
        ...
        assert_eq!(parsed.platform, "imessage");
    ...
    fn add_authorized_root_allows_runtime_export_staging() {
        ...
        assert_eq!(staged[0].platform_id, "imessage");
    }
    ...
    fn add_authorized_root_allows_runtime_media_inventory_collection() {
        ...
        let mut service = IMessageService::new(IMessageConfig::with_authorized_roots([
    ...
    fn add_authorized_root_allows_runtime_audio_output_validation_to_progress() {
        ...
        let result =
            service.transcode_audio_asset_to_mp3(&input, &output, &AudioTranscodeOptions::default());
        assert!(
            !matches!(result, Err(IMessageError::UnauthorizedPath { .. })),
    ...
    fn stage_authorized_exports_rejects_unauthorized_path_directly() {
        ...
        let unauthorized_dir = tempdir().expect("tempdir");
        let export = unauthorized_dir.path().join("imessage_fixture.txt");
        write_fixture(&export, r#"[{"text":"hello imessage","sender":"Alice","date":"2025-01-02T10:20:30Z"}]"#);

        let service = IMessageService::new(IMessageConfig::with_authorized_roots([
    ...
    fn stage_authorized_exports_preserves_source_path_and_platform_id() {
        let dir = tempdir().expect("tempdir");
        let export = dir.path().join("imessage_fixture.txt");
        write_fixture(&export, r#"[{"text":"hello imessage","sender":"Alice","date":"2025-01-02T10:20:30Z"}]"#);
