//! File monitoring for legal-safe iMessage export directories.
...
use crate::IMessageError;

/// File system event types emitted by the iMessage monitor.
...
/// File monitoring configuration for iMessage exports.
#[derive(Debug, Clone)]
...
/// File system monitor for iMessage export trees.
...
impl FileMonitor {
    ...
    pub fn new(config: FileMonitorConfig) -> Result<Self, IMessageError> {
    ...
    /// Start monitoring.
    pub fn start(&mut self) -> Result<(), IMessageError> {
    ...
    /// Stop monitoring.
    pub fn stop(&mut self) -> Result<(), IMessageError> {
    ...
    /// Default path patterns for iMessage export and media folders.
    pub fn imessage_export_patterns() -> Vec<Regex> {
        vec![
            Regex::new(r"(?i)\.(json|txt|zip|csv|html|db)$").expect("valid export regex"),
            Regex::new(r"(?i)\.(jpg|jpeg|png|heic|webp|gif|mp4|mov|ogg|opus|pdf|svg)$")
...
mod tests {
    ...
    fn export_patterns_match_common_assets() {
        let patterns = FileMonitor::imessage_export_patterns();
        let export_path = Path::new("/tmp/Export/chat.db");
        let image_path = Path::new("/tmp/Media/photo.heic");
