//! Account models for legal-safe IMessage contexts.
//!
...
/// Represents a discovered IMessage account context.
...
mod tests {
    ...
    fn source(kind: SourceKind, label: &str, path: &str) -> SourceCandidate {
        SourceCandidate {
            ...
            platform_id: "imessage".to_string(),
    ...
    fn builds_account_from_source() {
        let candidate = source(
            ...
            "macOS IMessage sandbox data",
            "/Users/test/Library/Containers/IMessage",
        );
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "macOS IMessage sandbox data");
    ...
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/imessage-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "imessage-export");
    }

    #[test]
    fn root_path_reflects_data_dir() {
        let candidate = source(SourceKind::ExportDirectory, "Archive", "/tmp/imessage-root");
        let account = Account::from_source(&candidate);

        assert_eq!(account.root_path(), Path::new("/tmp/imessage-root"));
    }
