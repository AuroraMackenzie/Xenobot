//! Media helpers for legal-safe iMessage export workflows.

use std::fs;
use std::path::{Path, PathBuf};

/// Media kind inferred from an iMessage export asset path.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum IMessageMediaKind {
    /// Image asset.
    Image,
    /// Video asset.
    Video,
...
pub struct IMessageMediaAsset {
    /// Original asset path.
    pub path: PathBuf,
    /// Inferred media kind.
    pub kind: IMessageMediaKind,
...
pub fn classify_media_path(path: &Path) -> IMessageMediaKind {
    let ext = path
        .extension()
    ...
    match ext.as_deref() {
        Some("jpg" | "jpeg" | "png" | "gif" | "bmp" | "heic" | "webp") => IMessageMediaKind::Image,
        Some("mp4" | "mov" | "avi" | "mkv" | "webm") => IMessageMediaKind::Video,
        Some("ogg" | "opus" | "mp3" | "m4a" | "wav" | "aac") => IMessageMediaKind::Audio,
        Some("pdf" | "doc" | "docx" | "xls" | "xlsx" | "ppt" | "pptx" | "txt" | "csv") => {
            IMessageMediaKind::Document
        }
        Some("zip" | "7z" | "rar" | "tar" | "gz") => IMessageMediaKind::Archive,
        Some("svg") => IMessageMediaKind::Sticker,
        _ => IMessageMediaKind::Unknown,
    }
}

/// Build a media inventory from explicitly provided asset paths.
pub fn collect_media_assets<I, P>(paths: I) -> Vec<IMessageMediaAsset>
where
...
mod tests {
    ...
    fn collects_existing_media_assets() {
        ...
        assert_eq!(assets[0].kind, IMessageMediaKind::Image);
