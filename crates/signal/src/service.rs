//! Service layer for legal-safe Signal ingestion workflows.
...
use crate::media::{collect_media_assets, SignalMediaAsset};
use crate::monitor::{FileMonitor, FileMonitorConfig};
use crate::{SignalAdapter, SignalError};

/// Parsed export staged by the Signal service.
#[derive(Debug, Clone)]
pub struct StagedSignalExport {
...
/// Aggregated legal-safe workspace assembled from explicit Signal inputs.
#[derive(Debug, Clone)]
pub struct AuthorizedSignalWorkspace {
    /// Stable platform identifier.
    pub platform_id: &'static str,
...
/// Legal-safe Signal orchestration service.
...
impl SignalService {
    ...
    pub fn stage_authorized_exports<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<StagedSignalExport>, SignalError>
    where
    ...
    pub fn collect_media_inventory<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<SignalMediaAsset>, SignalError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
    {
        let resolved: Result<Vec<PathBuf>, SignalError> = paths
            .into_iter()
            .map(|path| {
    ...
    pub fn build_authorized_workspace<I, P, J, Q>(
        ...
        export_paths: I,
        media_paths: J,
    ) -> Result<AuthorizedSignalWorkspace, SignalError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
    ...
    ) -> Result<(AuthorizedSignalWorkspace, Option<FileMonitor>), SignalError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        J: IntoIterator<Item = Q>,
    ...
    pub fn create_export_monitor(&self, watch_dir: &Path) -> Result<FileMonitor, SignalError> {
        self.ensure_authorized(watch_dir)?;
        FileMonitor::new(FileMonitorConfig {
            watch_dir: watch_dir.to_path_buf(),
            file_patterns: FileMonitor::signal_export_patterns(),
            recursive: true,
        })
    }

    fn ensure_authorized(&self, path: &Path) -> Result<(), SignalError> {
        if self.config.is_authorized_path(path) {
            Ok(())
        } else {
            Err(SignalError::UnauthorizedPath {
...
mod tests {
    ...
    fn returns_platform_id_from_service() {
        let service = SignalService::new(SignalConfig::default());
        assert_eq!(service.platform_id(), "signal");
    }

    #[test]
    fn rejects_paths_outside_authorized_roots() {
        let service = SignalService::new(SignalConfig::with_authorized_roots([PathBuf::from("/tmp/allowed")]));
        ...
        match err {
            SignalError::UnauthorizedPath { path } => {
    ...
    fn builds_media_inventory_for_authorized_assets() {
        ...
        let service = SignalService::new(SignalConfig::with_authorized_roots([dir.path().to_path_buf()]));
    ...
    fn build_authorized_workspace_rejects_unauthorized_media_paths() {
        ...
        let export = export_dir.path().join("signal_fixture.json");
        let media = media_dir.path().join("preview.gif");
        write_fixture(&export, r#"[{"sender":"Alice","timestamp":1735813230000,"body":"hello signal"}]"#);
        std::fs::write(&media, [1_u8, 2, 3]).expect("media fixture");

        let service =
            SignalService::new(SignalConfig::with_authorized_roots([export_dir.path().to_path_buf()]));

        match service.build_authorized_workspace([export.as_path()], [media.as_path()]) {
            Err(SignalError::UnauthorizedPath { path }) => assert_eq!(path, media),
    ...
    fn build_authorized_workspace_rejects_unauthorized_export_paths() {
        ...
        let export = unauthorized_dir.path().join("signal_unauthorized_fixture.dat");
        ...
        match service.build_authorized_workspace([export.as_path()], std::iter::empty::<&Path>()) {
            Err(SignalError::UnauthorizedPath { path }) => assert_eq!(path, export),
    ...
    fn builds_authorized_workspace_from_exports_and_media() {
        let dir = tempdir().expect("tempdir");
        let export = dir.path().join("signal_fixture.json");
        let asset = dir.path().join("voice.ogg");
        ...
        let service =
            SignalService::new(SignalConfig::with_authorized_roots([dir.path().to_path_buf()]));
        let workspace = service
            .build_authorized_workspace([export.as_path()], [asset.as_path()])
            .expect("workspace should build");

        assert_eq!(workspace.platform_id, "signal");
    ...
    fn prepare_authorized_workspace_rejects_unauthorized_watch_directory() {
        ...
        let export = input_dir.path().join("signal_fixture.json");
        write_fixture(
            &export,
            r#"[{"sender":"Alice","timestamp":1735813230000,"body":"hello signal"}]"#,
        );
        ...
        let service =
            SignalService::new(SignalConfig::with_authorized_roots([input_dir.path().to_path_buf()]));
        match service.prepare_authorized_workspace(
            [export.as_path()],
            std::iter::empty::<&Path>(),
            Some(watch_dir.path()),
        ) {
            Err(SignalError::UnauthorizedPath { path }) => {
    ...
    fn authorized_workspace_reports_empty_when_no_exports_or_media_are_staged() {
        let service = SignalService::new(SignalConfig::with_authorized_roots([std::env::temp_dir()]));
        let workspace = service
    ...
    fn rejects_audio_asset_transcoding_when_output_directory_is_not_authorized() {
        ...
        let service = SignalService::new(SignalConfig::with_authorized_roots([input_dir.path().to_path_buf()]));
        let error = service
    ...
    fn rejects_audio_asset_transcoding_when_input_path_is_not_authorized() {
        ...
        let service = SignalService::new(SignalConfig::with_authorized_roots([
    ...
    fn add_authorized_root_allows_runtime_monitor_creation() {
        ...
        let mut service = SignalService::new(SignalConfig::with_authorized_roots([
    ...
    fn collect_media_inventory_rejects_unauthorized_assets() {
        let media_dir = tempdir().expect("tempdir");
        let other_dir = tempdir().expect("tempdir");
        let asset = media_dir.path().join("preview.gif");
        std::fs::write(&asset, [1_u8, 2, 3]).expect("media fixture");

        let mut service =
            SignalService::new(SignalConfig::with_authorized_roots([other_dir.path().to_path_buf()]));
        match service.collect_media_inventory([asset.as_path()]) {
            Err(SignalError::UnauthorizedPath { path }) => assert_eq!(path, asset),
    ...
    fn add_authorized_root_allows_runtime_workspace_preparation_with_monitor() {
        ...
        let other_dir = tempdir().expect("tempdir");
        let export = dir.path().join("signal_fixture.json");
        std::fs::write(&export, r#"[{"sender":"Alice","timestamp":1735813230000,"body":"hello signal"}]"#).expect("fixture");
    ...
    fn add_authorized_root_allows_runtime_audio_input_validation_to_progress() {
        ...
        match service.transcode_audio_asset_to_mp3(&input, &output, &AudioTranscodeOptions::default()) {
            Err(SignalError::UnauthorizedPath { path }) => assert_eq!(path, input),
            _ => panic!("expected unauthorized path before runtime authorization"),
        }

        service.add_authorized_root(input_dir.path().to_path_buf());
        if let Err(SignalError::UnauthorizedPath { path }) =
    ...
    fn add_authorized_root_allows_runtime_export_parsing() {
        ...
        assert_eq!(parsed.platform, "signal");
    ...
    fn add_authorized_root_allows_runtime_export_staging() {
        ...
        assert_eq!(staged[0].platform_id, "signal");
    }
    ...
    fn add_authorized_root_allows_runtime_audio_output_validation_to_progress() {
        ...
        let result =
            service.transcode_audio_asset_to_mp3(&input, &output, &AudioTranscodeOptions::default());
        assert!(
            !matches!(result, Err(SignalError::UnauthorizedPath { .. })),
    ...
    fn stage_authorized_exports_rejects_unauthorized_path_directly() {
        ...
        let unauthorized_dir = tempdir().expect("tempdir");
        let export = unauthorized_dir.path().join("signal_fixture.txt");
        write_fixture(&export, r#"[{"sender":"Alice","timestamp":1735813230000,"body":"hello signal"}]"#);
    ...
    fn stage_authorized_exports_preserves_source_path_and_platform_id() {
        let dir = tempdir().expect("tempdir");
        let export = dir.path().join("signal_fixture.txt");
        write_fixture(&export, r#"[{"sender":"Alice","timestamp":1735813230000,"body":"hello signal"}]"#);
