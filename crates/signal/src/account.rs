//! Account models for legal-safe Signal contexts.
//!
...
/// Represents a discovered Signal account context.
...
mod tests {
    ...
    fn builds_account_from_source() {
        let candidate = source(
            ...
            "macOS Signal sandbox data",
            "/Users/test/Library/Containers/Signal",
        );
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "macOS Signal sandbox data");
    ...
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/signal-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "signal-export");
    }

    #[test]
    fn root_path_reflects_data_dir() {
        let candidate = source(SourceKind::ExportDirectory, "Archive", "/tmp/signal-root");
        let account = Account::from_source(&candidate);

        assert_eq!(account.root_path(), Path::new("/tmp/signal-root"));
    }
}
