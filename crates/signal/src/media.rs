//! Media helpers for legal-safe Signal export workflows.

use std::fs;
use std::path::{Path, PathBuf};

/// Media kind inferred from a Signal export asset path.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SignalMediaKind {
    /// Image asset.
    Image,
    /// Video asset.
    Video,
...
pub struct SignalMediaAsset {
    /// Original asset path.
    pub path: PathBuf,
    /// Inferred media kind.
    pub kind: SignalMediaKind,
...
pub fn classify_media_path(path: &Path) -> SignalMediaKind {
    let ext = path
        .extension()
    ...
    match ext.as_deref() {
        Some("jpg" | "jpeg" | "png" | "gif" | "bmp" | "heic" | "webp") => SignalMediaKind::Image,
        Some("mp4" | "mov" | "avi" | "mkv" | "webm") => SignalMediaKind::Video,
        Some("ogg" | "opus" | "mp3" | "m4a" | "wav" | "aac") => SignalMediaKind::Audio,
        Some("pdf" | "doc" | "docx" | "xls" | "xlsx" | "ppt" | "pptx" | "txt" | "csv") => {
            SignalMediaKind::Document
        }
        Some("zip" | "7z" | "rar" | "tar" | "gz") => SignalMediaKind::Archive,
        Some("svg") => SignalMediaKind::Sticker,
        _ => SignalMediaKind::Unknown,
    }
}

/// Build a media inventory from explicitly provided asset paths.
pub fn collect_media_assets<I, P>(paths: I) -> Vec<SignalMediaAsset>
where
...
mod tests {
    ...
    fn collects_existing_media_assets() {
        ...
        assert_eq!(assets[0].kind, SignalMediaKind::Image);
        assert_eq!(assets[0].size_bytes, 3);
    }

    #[test]
    fn classifies_unknown_extensions_as_unknown() {
        assert_eq!(classify_media_path(Path::new("mystery.bin")), SignalMediaKind::Unknown);
    }
