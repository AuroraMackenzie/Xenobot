//! Legal-safe LINE adapter for Xenobot.
//!
...
/// Legal-safe adapter for LINE workflows.
...
impl LineAdapter {
    ...
    pub fn parse_authorized_export(&self, path: &Path) -> Result<ParsedChat, LineError> {
        ...
        let parsed = registry.detect_and_parse(path).map_err(LineError::Parse)?;

        if parsed.platform.eq_ignore_ascii_case(PLATFORM_ID) {
            Ok(parsed)
        } else {
            Err(LineError::PlatformMismatch {
...
/// Errors returned by the LINE adapter.
#[derive(Debug, Error)]
pub enum LineError {
