//! Service layer for legal-safe LINE ingestion workflows.
...
use crate::media::{collect_media_assets, LineMediaAsset};
use crate::monitor::{FileMonitor, FileMonitorConfig};
use crate::{LineAdapter, LineError};

/// Parsed export staged by the LINE service.
#[derive(Debug, Clone)]
pub struct StagedLineExport {
...
/// Aggregated legal-safe workspace assembled from explicit LINE inputs.
#[derive(Debug, Clone)]
pub struct AuthorizedLineWorkspace {
    /// Stable platform identifier.
    pub platform_id: &'static str,
...
/// Legal-safe LINE orchestration service.
...
impl LineService {
    ...
    pub fn parse_authorized_export(&self, path: &Path) -> Result<ParsedChat, LineError> {
    ...
    where
        ...
        P: AsRef<Path>,
    {
        let resolved: Result<Vec<PathBuf>, LineError> = paths
            .into_iter()
            .map(|path| {
    ...
    pub fn build_authorized_workspace<I, P, J, Q>(
        ...
        export_paths: I,
        media_paths: J,
    ) -> Result<AuthorizedLineWorkspace, LineError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
    ...
    ) -> Result<(AuthorizedLineWorkspace, Option<FileMonitor>), LineError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        J: IntoIterator<Item = Q>,
    ...
    pub fn create_export_monitor(&self, watch_dir: &Path) -> Result<FileMonitor, LineError> {
        self.ensure_authorized(watch_dir)?;
        FileMonitor::new(FileMonitorConfig {
            watch_dir: watch_dir.to_path_buf(),
            file_patterns: FileMonitor::line_export_patterns(),
            recursive: true,
        })
    }

    fn ensure_authorized(&self, path: &Path) -> Result<(), LineError> {
...
mod tests {
    ...
    fn returns_platform_id_from_service() {
        let service = LineService::new(LineConfig::default());
        assert_eq!(service.platform_id(), "line");
    }

    #[test]
    fn rejects_paths_outside_authorized_roots() {
        let service = LineService::new(LineConfig::with_authorized_roots([PathBuf::from(
            "/tmp/allowed",
        )]));
        ...
        match err {
            LineError::UnauthorizedPath { path } => {
    ...
    fn build_authorized_workspace_rejects_unauthorized_media_paths() {
        ...
        let export = export_dir.path().join("line_fixture.txt");
    ...
    fn builds_authorized_workspace_from_exports_and_media() {
        ...
        assert_eq!(workspace.platform_id, "line");
    ...
    fn authorized_workspace_reports_empty_when_no_exports_or_media_are_staged() {
        let service = LineService::new(LineConfig::with_authorized_roots([std::env::temp_dir()]));
        let workspace = service
    ...
    fn collect_media_inventory_rejects_unauthorized_assets() {
        ...
        match error {
            LineError::UnauthorizedPath { path } => assert_eq!(path, asset),
            other => panic!("unexpected error: {other:?}"),
        }
    }
