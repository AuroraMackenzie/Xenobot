//! Account models for legal-safe LINE contexts.
//!
...
/// Represents a discovered LINE account context.
...
mod tests {
    ...
    fn builds_account_from_source() {
        let candidate = source(
            ...
            "LINE desktop app data",
            "/Users/test/Library/Application Support/LINE",
        );
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "LINE desktop app data");
    ...
    fn deduplicates_accounts_by_root_path() {
        let candidates = vec![
            source(SourceKind::ExportDirectory, "Downloads export", "/tmp/line"),
            source(SourceKind::AppContainer, "App data", "/tmp/line"),
        ];
    ...
    fn picks_primary_account_preferring_current() {
        ...
        assert_eq!(account.name, "App data");
        assert!(account.is_current);
    }
    #[test]
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/line-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "line-export");
    }

    #[test]
    fn root_path_reflects_data_dir() {
        let candidate = source(SourceKind::ExportDirectory, "Archive", "/tmp/line-root");
        let account = Account::from_source(&candidate);

        assert_eq!(account.root_path(), Path::new("/tmp/line-root"));
    }
}
