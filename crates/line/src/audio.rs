//! Audio transcoding helpers for legal-safe LINE media workflows.
//!
...
use crate::{LineError, LineResult};
use std::io::Write;
...
pub fn transcode_audio_to_mp3(
    ...
    if let Some(parent) = output_path.parent() {
        std::fs::create_dir_all(parent).map_err(LineError::Io)?;
    }
    ...
    Err(LineError::Internal(anyhow::anyhow!(
...
pub fn transcode_audio_bytes_to_mp3(
    ...
    if input_bytes.is_empty() {
        return Err(LineError::Internal(anyhow::anyhow!(
    ...
    if let Some(mut stdin) = child.stdin.take() {
        stdin.write_all(input_bytes).map_err(|error| {
            LineError::Internal(anyhow::anyhow!("failed to write ffmpeg stdin: {}", error))
        })?;
    }

    let output = child.wait_with_output().map_err(|error| {
        LineError::Internal(anyhow::anyhow!("failed to wait ffmpeg: {}", error))
    })?;
