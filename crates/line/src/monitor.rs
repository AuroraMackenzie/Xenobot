//! File monitoring for legal-safe LINE export directories.
...
/// File system event types emitted by the LINE monitor.
...
/// File monitoring configuration for LINE exports.
#[derive(Debug, Clone)]
...
/// File system monitor for LINE export trees.
...
impl FileMonitor {
    ...
    pub fn new(config: FileMonitorConfig) -> Result<Self, LineError> {
    ...
    /// Start monitoring.
    pub fn start(&mut self) -> Result<(), LineError> {
    ...
    /// Stop monitoring.
    pub fn stop(&mut self) -> Result<(), LineError> {
    ...
    /// Default path patterns for LINE export and media folders.
    pub fn line_export_patterns() -> Vec<Regex> {
        vec![
            Regex::new(r"(?i)\.(zip|txt|json|csv|html)$").expect("valid export regex"),
            Regex::new(r"(?i)\.(jpg|jpeg|png|webp|gif|mp4|mov|ogg|opus|pdf|svg)$")
...
mod tests {
    ...
    fn export_patterns_match_common_assets() {
        let patterns = FileMonitor::line_export_patterns();
        let export_path = Path::new("/tmp/Export/line.zip");
        let image_path = Path::new("/tmp/Media/sticker.svg");
