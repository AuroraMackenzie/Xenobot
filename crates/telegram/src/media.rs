//! Media helpers for legal-safe Telegram export workflows.

use std::fs;
use std::path::{Path, PathBuf};

/// Media kind inferred from a Telegram export asset path.
...
pub enum TelegramMediaKind {
    ...
    /// Video asset.
    Video,
    /// Audio or voice-note asset.
    Audio,
    /// Document or office file.
    Document,
    /// Archive file.
    Archive,
    /// Sticker or vector animation asset.
    Sticker,
...
pub fn classify_media_path(path: &Path) -> TelegramMediaKind {
    let ext = path
        .extension()
    ...
    match ext.as_deref() {
        Some("jpg" | "jpeg" | "png" | "gif" | "bmp" | "heic" | "webp") => TelegramMediaKind::Image,
        Some("mp4" | "mov" | "avi" | "mkv" | "webm") => TelegramMediaKind::Video,
        Some("ogg" | "opus" | "mp3" | "m4a" | "wav" | "aac" | "flac") => TelegramMediaKind::Audio,
        Some("pdf" | "doc" | "docx" | "xls" | "xlsx" | "ppt" | "pptx" | "txt" | "csv") => {
            TelegramMediaKind::Document
        }
        Some("zip" | "7z" | "rar" | "tar" | "gz") => TelegramMediaKind::Archive,
        Some("svg" | "tgs") => TelegramMediaKind::Sticker,
        _ => TelegramMediaKind::Unknown,
    }
}

/// Build a media inventory from explicitly provided asset paths.
pub fn collect_media_assets<I, P>(paths: I) -> Vec<TelegramMediaAsset>
where
...
mod tests {
    ...
    fn collects_existing_media_assets() {
        ...
        assert_eq!(assets[0].kind, TelegramMediaKind::Image);
