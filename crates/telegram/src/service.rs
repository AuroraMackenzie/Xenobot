//! Service layer for legal-safe Telegram ingestion workflows.
...
use crate::media::{collect_media_assets, TelegramMediaAsset};
use crate::monitor::{FileMonitor, FileMonitorConfig};
use crate::{TelegramAdapter, TelegramError};

/// Parsed export staged by the Telegram service.
#[derive(Debug, Clone)]
pub struct StagedTelegramExport {
...
/// Aggregated legal-safe workspace assembled from explicit Telegram inputs.
#[derive(Debug, Clone)]
pub struct AuthorizedTelegramWorkspace {
    /// Stable platform identifier.
    pub platform_id: &'static str,
...
/// Legal-safe Telegram orchestration service.
...
impl TelegramService {
    ...
    pub fn stage_authorized_exports<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<StagedTelegramExport>, TelegramError>
    where
    ...
    pub fn collect_media_inventory<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<TelegramMediaAsset>, TelegramError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
    {
        let resolved: Result<Vec<PathBuf>, TelegramError> = paths
            .into_iter()
            .map(|path| {
    ...
    pub fn build_authorized_workspace<I, P, J, Q>(
        ...
        export_paths: I,
        media_paths: J,
    ) -> Result<AuthorizedTelegramWorkspace, TelegramError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        ...
        Q: AsRef<Path>,
    {
        ...
        Ok(AuthorizedTelegramWorkspace {
            platform_id: self.platform_id(),
            accounts,
            primary_account,
            staged_exports,
    ...
    ) -> Result<(AuthorizedTelegramWorkspace, Option<FileMonitor>), TelegramError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        J: IntoIterator<Item = Q>,
    ...
    pub fn create_export_monitor(&self, watch_dir: &Path) -> Result<FileMonitor, TelegramError> {
        self.ensure_authorized(watch_dir)?;
        FileMonitor::new(FileMonitorConfig {
            watch_dir: watch_dir.to_path_buf(),
            file_patterns: FileMonitor::telegram_export_patterns(),
            recursive: true,
        })
    }

    fn ensure_authorized(&self, path: &Path) -> Result<(), TelegramError> {
...
mod tests {
    ...
    fn returns_platform_id_from_service() {
        ...
        assert_eq!(service.platform_id(), "telegram");
    }

    #[test]
    fn rejects_paths_outside_authorized_roots() {
        let service = TelegramService::new(TelegramConfig::with_authorized_roots([PathBuf::from(
            "/tmp/allowed",
        )]));
        ...
        match err {
            TelegramError::UnauthorizedPath { path } => {
    ...
    fn build_authorized_workspace_rejects_unauthorized_media_paths() {
        ...
        let export = export_dir.path().join("telegram_fixture.json");
    ...
    fn builds_authorized_workspace_from_exports_and_media() {
        ...
        assert_eq!(workspace.platform_id, "telegram");
    ...
    fn prepare_authorized_workspace_rejects_unauthorized_watch_directory() {
        ...
        let export = input_dir.path().join("telegram_fixture.json");
        write_fixture(
            &export,
            r#"{"name":"tg","messages":[{"from":"Alice","date":"2025-01-02T10:20:30Z","text":"hello telegram"}]}"#,
        );
    ...
    fn collect_media_inventory_rejects_unauthorized_assets() {
        ...
        match error {
            TelegramError::UnauthorizedPath { path } => assert_eq!(path, asset),
            other => panic!("unexpected error: {other:?}"),
        }
    }
