//! Account models for legal-safe Telegram contexts.
//!
...
/// Represents a discovered Telegram account context.
...
mod tests {
    ...
    fn builds_account_from_source() {
        let candidate = source(
            ...
            "Telegram desktop app data",
            "/Users/test/Library/Application Support/Telegram",
        );
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "Telegram desktop app data");
    ...
    fn deduplicates_accounts_by_root_path() {
        let candidates = vec![
            source(SourceKind::ExportDirectory, "Downloads export", "/tmp/tg"),
            source(SourceKind::AppContainer, "Desktop data", "/tmp/tg"),
        ];
    ...
    fn picks_primary_account_preferring_current() {
        ...
        assert_eq!(account.name, "Desktop data");
        assert!(account.is_current);
    }
    #[test]
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/telegram-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "telegram-export");
    }

    #[test]
    fn root_path_reflects_data_dir() {
        let candidate = source(SourceKind::ExportDirectory, "Archive", "/tmp/telegram-root");
        let account = Account::from_source(&candidate);
