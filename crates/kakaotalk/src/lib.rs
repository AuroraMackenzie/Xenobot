//! Legal-safe KakaoTalk adapter for Xenobot.
//!
...
/// Legal-safe adapter for KakaoTalk workflows.
#[derive(Debug, Clone, Copy, Default)]
pub struct KakaoTalkAdapter;

impl KakaoTalkAdapter {
    ...
    pub fn parse_authorized_export(&self, path: &Path) -> Result<ParsedChat, KakaoTalkError> {
        ...
        if parsed.platform.eq_ignore_ascii_case(PLATFORM_ID) {
            ...
            Err(KakaoTalkError::PlatformMismatch {
...
/// Errors returned by the KakaoTalk adapter.
#[derive(Debug, Error)]
pub enum KakaoTalkError {
