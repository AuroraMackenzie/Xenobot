//! Service layer for legal-safe KakaoTalk ingestion workflows.
...
use crate::config::KakaoTalkConfig;
use crate::media::{collect_media_assets, KakaoTalkMediaAsset};
use crate::monitor::{FileMonitor, FileMonitorConfig};
use crate::{KakaoTalkAdapter, KakaoTalkError};

/// Parsed export staged by the KakaoTalk service.
#[derive(Debug, Clone)]
pub struct StagedKakaoTalkExport {
...
/// Aggregated legal-safe workspace assembled from explicit KakaoTalk inputs.
#[derive(Debug, Clone)]
pub struct AuthorizedKakaoTalkWorkspace {
    /// Stable platform identifier.
    pub platform_id: &'static str,
...
/// Legal-safe KakaoTalk orchestration service.
#[derive(Debug, Clone, Default)]
pub struct KakaoTalkService {
    adapter: KakaoTalkAdapter,
    config: KakaoTalkConfig,
}

impl KakaoTalkService {
    /// Create a service from a runtime configuration.
    pub fn new(config: KakaoTalkConfig) -> Self {
        Self {
            adapter: KakaoTalkAdapter::new(),
            config,
        }
    }
    ...
    pub fn stage_authorized_exports<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<StagedKakaoTalkExport>, KakaoTalkError>
    where
    ...
    pub fn collect_media_inventory<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<KakaoTalkMediaAsset>, KakaoTalkError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
    {
        let resolved: Result<Vec<PathBuf>, KakaoTalkError> = paths
            .into_iter()
            .map(|path| {
    ...
    pub fn build_authorized_workspace<I, P, J, Q>(
        ...
        export_paths: I,
        media_paths: J,
    ) -> Result<AuthorizedKakaoTalkWorkspace, KakaoTalkError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        ...
        Q: AsRef<Path>,
    {
        ...
        Ok(AuthorizedKakaoTalkWorkspace {
            platform_id: self.platform_id(),
            accounts,
            primary_account,
            staged_exports,
    ...
    ) -> Result<(AuthorizedKakaoTalkWorkspace, Option<FileMonitor>), KakaoTalkError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        J: IntoIterator<Item = Q>,
    ...
    pub fn create_export_monitor(&self, watch_dir: &Path) -> Result<FileMonitor, KakaoTalkError> {
        self.ensure_authorized(watch_dir)?;
        FileMonitor::new(FileMonitorConfig {
            watch_dir: watch_dir.to_path_buf(),
            file_patterns: FileMonitor::kakaotalk_export_patterns(),
            recursive: true,
        })
    }

    fn ensure_authorized(&self, path: &Path) -> Result<(), KakaoTalkError> {
...
mod tests {
    ...
    fn returns_platform_id_from_service() {
        let service = KakaoTalkService::new(KakaoTalkConfig::default());
        assert_eq!(service.platform_id(), "kakaotalk");
    }
    ...
    fn rejects_paths_outside_authorized_roots() {
        ...
        match err {
            KakaoTalkError::UnauthorizedPath { path } => {
    ...
    fn builds_authorized_workspace_from_exports_and_media() {
        ...
        assert_eq!(workspace.platform_id, "kakaotalk");
    ...
    fn prepare_authorized_workspace_rejects_unauthorized_watch_directory() {
        ...
        let export = input_dir.path().join("kakaotalk_fixture.json");
        write_fixture(
            &export,
            r#"[{"sender":"Alice","message":"hello kakao","date":"2025-01-02 10:20:30"}]"#,
        );
    ...
    fn add_authorized_root_allows_runtime_audio_input_validation_to_progress() {
        ...
        if let Err(KakaoTalkError::UnauthorizedPath { path }) =
    ...
    fn add_authorized_root_allows_runtime_export_parsing() {
        ...
        assert_eq!(parsed.platform, "kakaotalk");
