//! Account models for legal-safe KakaoTalk contexts.
//!
...
/// Represents a discovered KakaoTalk account context.
...
fn normalize_label(label: &str, path: &Path) -> String {
    ...
    path.file_name()
        ...
        .unwrap_or_else(|| "KakaoTalk".to_string())
}
...
mod tests {
    ...
    fn source(kind: SourceKind, label: &str, path: &str) -> SourceCandidate {
        SourceCandidate {
            platform: Platform::KakaoTalk,
            platform_id: "kakaotalk".to_string(),
    ...
    fn builds_account_from_source() {
        let candidate = source(
            ...
            "macOS KakaoTalk sandbox data",
            "/Users/test/Library/Containers/KakaoTalk",
        );
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "macOS KakaoTalk sandbox data");
    ...
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/kakaotalk-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "kakaotalk-export");
    }

    #[test]
    fn root_path_reflects_data_dir() {
        let candidate = source(SourceKind::ExportDirectory, "Archive", "/tmp/kakaotalk-root");
        let account = Account::from_source(&candidate);

        assert_eq!(account.root_path(), Path::new("/tmp/kakaotalk-root"));
    }
