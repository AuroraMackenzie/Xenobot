//! Account models for legal-safe Viber contexts.
//!
...
/// Represents a discovered Viber account context.
...
fn normalize_label(label: &str, path: &Path) -> String {
    ...
    path.file_name()
        ...
        .unwrap_or_else(|| "Viber".to_string())
}
...
mod tests {
    ...
    fn builds_account_from_source() {
        let candidate = source(
            ...
            "macOS Viber sandbox data",
            "/Users/test/Library/Containers/Viber",
        );
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "macOS Viber sandbox data");
    ...
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/viber-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "viber-export");
    }

    #[test]
    fn root_path_reflects_data_dir() {
        let candidate = source(SourceKind::ExportDirectory, "Archive", "/tmp/viber-root");
        let account = Account::from_source(&candidate);

        assert_eq!(account.root_path(), Path::new("/tmp/viber-root"));
    }
