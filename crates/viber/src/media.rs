//! Media helpers for legal-safe Viber export workflows.

use std::fs;
use std::path::{Path, PathBuf};

/// Media kind inferred from a Viber export asset path.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ViberMediaKind {
    /// Image asset.
    Image,
    /// Video asset.
    Video,
...
pub struct ViberMediaAsset {
    /// Original asset path.
    pub path: PathBuf,
    /// Inferred media kind.
    pub kind: ViberMediaKind,
...
pub fn classify_media_path(path: &Path) -> ViberMediaKind {
    let ext = path
        .extension()
    ...
    match ext.as_deref() {
        Some("jpg" | "jpeg" | "png" | "gif" | "bmp" | "heic" | "webp") => ViberMediaKind::Image,
        Some("mp4" | "mov" | "avi" | "mkv" | "webm") => ViberMediaKind::Video,
        Some("ogg" | "opus" | "mp3" | "m4a" | "wav" | "aac") => ViberMediaKind::Audio,
        Some("pdf" | "doc" | "docx" | "xls" | "xlsx" | "ppt" | "pptx" | "txt" | "csv") => {
            ViberMediaKind::Document
        }
        Some("zip" | "7z" | "rar" | "tar" | "gz") => ViberMediaKind::Archive,
        Some("svg") => ViberMediaKind::Sticker,
        _ => ViberMediaKind::Unknown,
    }
}

/// Build a media inventory from explicitly provided asset paths.
pub fn collect_media_assets<I, P>(paths: I) -> Vec<ViberMediaAsset>
where
...
mod tests {
    ...
    fn collects_existing_media_assets() {
        ...
        assert_eq!(assets[0].kind, ViberMediaKind::Image);
        assert_eq!(assets[0].size_bytes, 3);
    }

    #[test]
    fn classifies_unknown_extensions_as_unknown() {
        assert_eq!(classify_media_path(Path::new("mystery.bin")), ViberMediaKind::Unknown);
    }
