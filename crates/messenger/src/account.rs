//! Account models for legal-safe Messenger contexts.
//!
...
/// Represents a discovered Messenger account context.
...
mod tests {
    ...
    fn source(kind: SourceKind, label: &str, path: &str) -> SourceCandidate {
        SourceCandidate {
            ...
            platform_id: "messenger".to_string(),
    ...
    fn builds_account_from_source() {
        let candidate = source(
            ...
            "macOS Messenger sandbox data",
            "/Users/test/Library/Containers/Messenger",
        );
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "macOS Messenger sandbox data");
    ...
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/messenger-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "messenger-export");
    }
