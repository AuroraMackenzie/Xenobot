//! Legal-safe Messenger adapter for Xenobot.
//!
...
pub use service::{AuthorizedMessengerWorkspace, MessengerService, StagedMessengerExport};
/// Common result type used by Messenger crate operations.
pub type MessengerResult<T> = Result<T, MessengerError>;

/// Stable platform identifier.
pub const PLATFORM_ID: &str = "messenger";

/// Legal-safe adapter for Messenger workflows.
...
impl MessengerAdapter {
    ...
    pub fn parse_authorized_export(&self, path: &Path) -> Result<ParsedChat, MessengerError> {
        ...
        let parsed = registry
            ...
            .map_err(MessengerError::Parse)?;

        if parsed.platform.eq_ignore_ascii_case(PLATFORM_ID) {
            Ok(parsed)
        } else {
            Err(MessengerError::PlatformMismatch {
...
/// Errors returned by the Messenger adapter.
#[derive(Debug, Error)]
