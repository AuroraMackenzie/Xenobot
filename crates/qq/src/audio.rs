//! Audio transcoding helpers for legal-safe QQ media workflows.
//!
...
pub fn transcode_audio_to_mp3(
    ...
    if !input_path.exists() {
        return Err(QQError::Io(std::io::Error::new(
    ...
    if let Some(parent) = output_path.parent() {
        std::fs::create_dir_all(parent).map_err(QQError::Io)?;
    }
    ...
    let output = Command::new(binary)
        ...
        .output()
        .map_err(|error| QQError::Internal(anyhow::anyhow!("failed to start ffmpeg: {}", error)))?;
    ...
    Err(QQError::Internal(anyhow::anyhow!(
...
pub fn transcode_audio_bytes_to_mp3(
    ...
    if input_bytes.is_empty() {
        return Err(QQError::Internal(anyhow::anyhow!(
    ...
    if let Some(mut stdin) = child.stdin.take() {
        stdin.write_all(input_bytes).map_err(|error| {
            QQError::Internal(anyhow::anyhow!("failed to write ffmpeg stdin: {}", error))
        })?;
    }
