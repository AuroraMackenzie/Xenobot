//! Service layer for legal-safe QQ ingestion workflows.
...
use crate::{QQAdapter, QQError};

/// Parsed export staged by the QQ service.
#[derive(Debug, Clone)]
pub struct StagedQQExport {
...
/// Aggregated legal-safe workspace assembled from explicit QQ inputs.
#[derive(Debug, Clone)]
pub struct AuthorizedQQWorkspace {
    /// Stable platform identifier.
    pub platform_id: &'static str,
...
/// Legal-safe QQ orchestration service.
...
impl QQService {
    ...
    pub fn stage_authorized_exports<I, P>(&self, paths: I) -> Result<Vec<StagedQQExport>, QQError>
    where
    ...
    where
        ...
        P: AsRef<Path>,
    {
        let resolved: Result<Vec<PathBuf>, QQError> = paths
            .into_iter()
            .map(|path| {
    ...
    pub fn build_authorized_workspace<I, P, J, Q>(
        ...
        export_paths: I,
        media_paths: J,
    ) -> Result<AuthorizedQQWorkspace, QQError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
    ...
    ) -> Result<(AuthorizedQQWorkspace, Option<FileMonitor>), QQError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        J: IntoIterator<Item = Q>,
    ...
    pub fn create_export_monitor(&self, watch_dir: &Path) -> Result<FileMonitor, QQError> {
    ...
    fn ensure_authorized(&self, path: &Path) -> Result<(), QQError> {
...
mod tests {
    ...
    fn returns_platform_id_from_service() {
        let service = QQService::new(QQConfig::default());
        assert_eq!(service.platform_id(), "qq");
    }

    #[test]
    fn rejects_paths_outside_authorized_roots() {
        let service = QQService::new(QQConfig::with_authorized_roots([PathBuf::from(
            "/tmp/allowed",
        )]));
    ...
    fn builds_media_inventory_for_authorized_assets() {
        ...
        let service = QQService::new(QQConfig::with_authorized_roots([dir.path().to_path_buf()]));
    ...
    fn build_authorized_workspace_rejects_unauthorized_media_paths() {
        ...
        let export = export_dir.path().join("qq_fixture.txt");
    ...
    fn builds_authorized_workspace_from_exports_and_media() {
        ...
        assert_eq!(workspace.platform_id, "qq");
    ...
    fn authorized_workspace_reports_empty_when_no_exports_or_media_are_staged() {
        let service = QQService::new(QQConfig::with_authorized_roots([std::env::temp_dir()]));
        let workspace = service
    ...
    fn collect_media_inventory_rejects_unauthorized_assets() {
        ...
        match error {
            QQError::UnauthorizedPath { path } => assert_eq!(path, asset),
            other => panic!("unexpected error: {other:?}"),
        }
    }
    ...
    fn add_authorized_root_allows_runtime_audio_output_validation_to_progress() {
        let input_dir = tempdir().expect("tempdir");
        let output_dir = tempdir().expect("tempdir");
        let input = input_dir.path().join("voice.amr");
        let output = output_dir.path().join("voice.mp3");
