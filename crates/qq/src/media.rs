//! Media helpers for legal-safe QQ export workflows.

use std::fs;
use std::path::{Path, PathBuf};

/// Media kind inferred from a QQ export asset path.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum QQMediaKind {
    /// Image asset.
    Image,
    /// Video asset.
    Video,
...
pub struct QQMediaAsset {
    /// Original asset path.
    pub path: PathBuf,
    /// Inferred media kind.
    pub kind: QQMediaKind,
...
pub fn classify_media_path(path: &Path) -> QQMediaKind {
    let ext = path
        .extension()
    ...
    match ext.as_deref() {
        Some("jpg" | "jpeg" | "png" | "gif" | "bmp" | "heic" | "webp") => QQMediaKind::Image,
        Some("mp4" | "mov" | "avi" | "mkv" | "webm") => QQMediaKind::Video,
        Some("ogg" | "opus" | "mp3" | "m4a" | "wav" | "aac") => QQMediaKind::Audio,
        Some("pdf" | "doc" | "docx" | "xls" | "xlsx" | "ppt" | "pptx" | "txt" | "csv") => {
            QQMediaKind::Document
        }
        Some("zip" | "7z" | "rar" | "tar" | "gz") => QQMediaKind::Archive,
        Some("svg") => QQMediaKind::Sticker,
        _ => QQMediaKind::Unknown,
    }
}

/// Build a media inventory from explicitly provided asset paths.
pub fn collect_media_assets<I, P>(paths: I) -> Vec<QQMediaAsset>
where
...
mod tests {
    ...
    fn collects_existing_media_assets() {
        ...
        assert_eq!(assets[0].kind, QQMediaKind::Image);
