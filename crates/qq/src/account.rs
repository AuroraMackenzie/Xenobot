//! Account models for legal-safe QQ contexts.
//!
...
/// Represents a discovered QQ account context.
...
mod tests {
    ...
    fn builds_account_from_source() {
        let candidate = source(
            ...
            "QQ sandbox data",
            "/Users/test/Library/Containers/com.tencent.qq",
        );
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "QQ sandbox data");
    ...
    fn deduplicates_accounts_by_root_path() {
        let candidates = vec![
            source(SourceKind::ExportDirectory, "Downloads export", "/tmp/qq"),
            source(SourceKind::AppContainer, "App data", "/tmp/qq"),
        ];
    ...
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/qq-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "qq-export");
    }

    #[test]
    fn root_path_reflects_data_dir() {
        let candidate = source(SourceKind::ExportDirectory, "Archive", "/tmp/qq-root");
        let account = Account::from_source(&candidate);

        assert_eq!(account.root_path(), Path::new("/tmp/qq-root"));
    }
