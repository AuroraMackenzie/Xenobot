//! File monitoring for legal-safe Google Chat export directories.
...
use crate::GoogleChatError;

/// File system event types emitted by the Google Chat monitor.
...
/// File monitoring configuration for Google Chat exports.
#[derive(Debug, Clone)]
...
/// File system monitor for Google Chat export trees.
...
impl FileMonitor {
    ...
    pub fn new(config: FileMonitorConfig) -> Result<Self, GoogleChatError> {
    ...
    /// Start monitoring.
    pub fn start(&mut self) -> Result<(), GoogleChatError> {
    ...
    /// Stop monitoring.
    pub fn stop(&mut self) -> Result<(), GoogleChatError> {
    ...
    /// Default path patterns for Google Chat export and media folders.
    pub fn googlechat_export_patterns() -> Vec<Regex> {
        vec![
            Regex::new(r"(?i)\.(json|txt|zip|csv|html)$").expect("valid export regex"),
...
mod tests {
    ...
    fn export_patterns_match_common_assets() {
        let patterns = FileMonitor::googlechat_export_patterns();
