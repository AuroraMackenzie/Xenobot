//! Service layer for legal-safe Google Chat ingestion workflows.
...
use crate::media::{collect_media_assets, GoogleChatMediaAsset};
use crate::monitor::{FileMonitor, FileMonitorConfig};
use crate::{GoogleChatAdapter, GoogleChatError};

/// Parsed export staged by the Google Chat service.
#[derive(Debug, Clone)]
pub struct StagedGoogleChatExport {
...
/// Aggregated legal-safe workspace assembled from explicit Google Chat inputs.
#[derive(Debug, Clone)]
pub struct AuthorizedGoogleChatWorkspace {
    /// Stable platform identifier.
    pub platform_id: &'static str,
...
/// Legal-safe Google Chat orchestration service.
...
impl GoogleChatService {
    ...
    pub fn stage_authorized_exports<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<StagedGoogleChatExport>, GoogleChatError>
    where
    ...
    pub fn collect_media_inventory<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<GoogleChatMediaAsset>, GoogleChatError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
    {
        let resolved: Result<Vec<PathBuf>, GoogleChatError> = paths
            .into_iter()
            .map(|path| {
    ...
    pub fn build_authorized_workspace<I, P, J, Q>(
        ...
        export_paths: I,
        media_paths: J,
    ) -> Result<AuthorizedGoogleChatWorkspace, GoogleChatError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        ...
        Q: AsRef<Path>,
    {
        ...
        Ok(AuthorizedGoogleChatWorkspace {
            platform_id: self.platform_id(),
            accounts,
            primary_account,
            staged_exports,
    ...
    ) -> Result<(AuthorizedGoogleChatWorkspace, Option<FileMonitor>), GoogleChatError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        J: IntoIterator<Item = Q>,
    ...
    pub fn create_export_monitor(&self, watch_dir: &Path) -> Result<FileMonitor, GoogleChatError> {
        self.ensure_authorized(watch_dir)?;
        FileMonitor::new(FileMonitorConfig {
            watch_dir: watch_dir.to_path_buf(),
            file_patterns: FileMonitor::googlechat_export_patterns(),
            recursive: true,
        })
    }

    fn ensure_authorized(&self, path: &Path) -> Result<(), GoogleChatError> {
        if self.config.is_authorized_path(path) {
            Ok(())
        } else {
            Err(GoogleChatError::UnauthorizedPath {
...
mod tests {
    ...
    fn returns_platform_id_from_service() {
        let service = GoogleChatService::new(GoogleChatConfig::default());
        assert_eq!(service.platform_id(), "googlechat");
    }

    #[test]
    fn rejects_paths_outside_authorized_roots() {
        let service = GoogleChatService::new(GoogleChatConfig::with_authorized_roots([PathBuf::from("/tmp/allowed")]));
        ...
        match err {
            GoogleChatError::UnauthorizedPath { path } => {
    ...
    fn builds_media_inventory_for_authorized_assets() {
        ...
        let service = GoogleChatService::new(GoogleChatConfig::with_authorized_roots([dir.path().to_path_buf()]));
    ...
    fn build_authorized_workspace_rejects_unauthorized_media_paths() {
        ...
        let export = export_dir.path().join("googlechat_fixture.json");
        let media = media_dir.path().join("preview.webp");
        write_fixture(&export, r#"[{"sender":{"name":"users/1","display_name":"Alice"},"create_time":"2025-01-02T10:20:30Z","text":"hello googlechat"}]"#);
        std::fs::write(&media, [1_u8, 2, 3]).expect("media fixture");

        let service =
            GoogleChatService::new(GoogleChatConfig::with_authorized_roots([export_dir.path().to_path_buf()]));

        match service.build_authorized_workspace([export.as_path()], [media.as_path()]) {
            Err(GoogleChatError::UnauthorizedPath { path }) => assert_eq!(path, media),
    ...
    fn build_authorized_workspace_rejects_unauthorized_export_paths() {
        ...
        let export = unauthorized_dir.path().join("googlechat_unauthorized_fixture.dat");
        ...
        match service.build_authorized_workspace([export.as_path()], std::iter::empty::<&Path>()) {
            Err(GoogleChatError::UnauthorizedPath { path }) => assert_eq!(path, export),
    ...
    fn builds_authorized_workspace_from_exports_and_media() {
        let dir = tempdir().expect("tempdir");
        let export = dir.path().join("googlechat_fixture.json");
        let asset = dir.path().join("voice.ogg");
        ...
        let service = GoogleChatService::new(GoogleChatConfig::with_authorized_roots([
        ...
        assert_eq!(workspace.platform_id, "googlechat");
    ...
    fn prepare_authorized_workspace_rejects_unauthorized_watch_directory() {
        ...
        let export = input_dir.path().join("googlechat_fixture.json");
        write_fixture(
            &export,
            r#"[{"sender":{"name":"users/1","display_name":"Alice"},"create_time":"2025-01-02T10:20:30Z","text":"hello googlechat"}]"#,
        );
        ...
        match service.prepare_authorized_workspace(
            ...
            Err(GoogleChatError::UnauthorizedPath { path }) => {
    ...
    fn authorized_workspace_reports_empty_when_no_exports_or_media_are_staged() {
        let service = GoogleChatService::new(GoogleChatConfig::with_authorized_roots([std::env::temp_dir()]));
        let workspace = service
    ...
    fn rejects_audio_asset_transcoding_when_output_directory_is_not_authorized() {
        ...
        let service = GoogleChatService::new(GoogleChatConfig::with_authorized_roots([input_dir.path().to_path_buf()]));
        let error = service
    ...
    fn add_authorized_root_allows_runtime_monitor_creation() {
        ...
        let mut service = GoogleChatService::new(GoogleChatConfig::with_authorized_roots([
    ...
    fn collect_media_inventory_rejects_unauthorized_assets() {
        let media_dir = tempdir().expect("tempdir");
        let other_dir = tempdir().expect("tempdir");
        let asset = media_dir.path().join("preview.webp");
        std::fs::write(&asset, [1_u8, 2, 3]).expect("media fixture");

        let mut service =
            GoogleChatService::new(GoogleChatConfig::with_authorized_roots([other_dir.path().to_path_buf()]));
        match service.collect_media_inventory([asset.as_path()]) {
            Err(GoogleChatError::UnauthorizedPath { path }) => assert_eq!(path, asset),
    ...
    fn add_authorized_root_allows_runtime_workspace_preparation_with_monitor() {
        ...
        let other_dir = tempdir().expect("tempdir");
        let export = dir.path().join("googlechat_fixture.json");
        std::fs::write(&export, r#"[{"sender":{"name":"users/1","display_name":"Alice"},"create_time":"2025-01-02T10:20:30Z","text":"hello googlechat"}]"#).expect("fixture");
    ...
    fn add_authorized_root_allows_runtime_audio_input_validation_to_progress() {
        ...
        match service.transcode_audio_asset_to_mp3(&input, &output, &AudioTranscodeOptions::default()) {
            Err(GoogleChatError::UnauthorizedPath { path }) => assert_eq!(path, input),
            _ => panic!("expected unauthorized path before runtime authorization"),
        }

        service.add_authorized_root(input_dir.path().to_path_buf());
        if let Err(GoogleChatError::UnauthorizedPath { path }) =
    ...
    fn add_authorized_root_allows_runtime_export_parsing() {
        ...
        assert_eq!(parsed.platform, "googlechat");
    ...
    fn add_authorized_root_allows_runtime_export_staging() {
        ...
        assert_eq!(staged[0].platform_id, "googlechat");
    }
    ...
    fn add_authorized_root_allows_runtime_audio_output_validation_to_progress() {
        ...
        let result =
            service.transcode_audio_asset_to_mp3(&input, &output, &AudioTranscodeOptions::default());
        assert!(
            !matches!(result, Err(GoogleChatError::UnauthorizedPath { .. })),
    ...
    fn stage_authorized_exports_rejects_unauthorized_path_directly() {
        ...
        let unauthorized_dir = tempdir().expect("tempdir");
        let export = unauthorized_dir.path().join("googlechat_fixture.txt");
        write_fixture(&export, r#"[{"sender":{"name":"users/1","display_name":"Alice"},"create_time":"2025-01-02T10:20:30Z","text":"hello googlechat"}]"#);
    ...
    fn stage_authorized_exports_preserves_source_path_and_platform_id() {
        let dir = tempdir().expect("tempdir");
        let export = dir.path().join("googlechat_fixture.txt");
        write_fixture(&export, r#"[{"sender":{"name":"users/1","display_name":"Alice"},"create_time":"2025-01-02T10:20:30Z","text":"hello googlechat"}]"#);
