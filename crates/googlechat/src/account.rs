//! Account models for legal-safe Google Chat contexts.
//!
...
/// Represents a discovered Google Chat account context.
...
fn normalize_label(label: &str, path: &Path) -> String {
    ...
    path.file_name()
        ...
        .unwrap_or_else(|| "Google Chat".to_string())
}
...
mod tests {
    ...
    fn source(kind: SourceKind, label: &str, path: &str) -> SourceCandidate {
        SourceCandidate {
            ...
            platform_id: "googlechat".to_string(),
    ...
    fn builds_account_from_source() {
        let candidate = source(
            ...
            "macOS Google Chat sandbox data",
            "/Users/test/Library/Containers/Google Chat",
        );
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "macOS Google Chat sandbox data");
    ...
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/googlechat-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "googlechat-export");
    }

    #[test]
    fn root_path_reflects_data_dir() {
        let candidate = source(SourceKind::ExportDirectory, "Archive", "/tmp/googlechat-root");
        let account = Account::from_source(&candidate);

        assert_eq!(account.root_path(), Path::new("/tmp/googlechat-root"));
    }
