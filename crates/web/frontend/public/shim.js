...
(function () {
  ...
  function normalizeImportProgressStatus(status) {
    ...
    if (s === "completed" || s === "done" || s === "success" || s === "idle")
      return "done";
  ...
  async function readImportProgressSnapshot() {
    const response = await fetch("/api/chat/import-progress", {
      method: "GET",
    });
    if (!response.ok) {
      throw new Error(`HTTP ${response.status}: ${response.statusText}`);
  ...
  function normalizeMentionAnalysis(raw) {
    ...
    const memberDetails = Array.from(detailMap.values()).map((detail) => ({
      ...detail,
      topMentioned: detail.topMentioned
        .sort((a, b) => b.count - a.count)
        .slice(0, 10),
      topMentioners: detail.topMentioners
        .sort((a, b) => b.count - a.count)
        .slice(0, 10),
    }));
    ...
    const totalMentions = toNumber(
      input.totalMentions,
      twoWay.reduce((sum, p) => sum + p.total, 0) +
        oneWay.reduce((sum, p) => sum + p.fromToCount, 0),
    );
  ...
  window.sessionApi = {
    generate: (sessionId, gapThreshold) => {
      ...
      return httpRequest(
        "POST",
        `/session/generate/${encodeURIComponent(sessionId)}`,
        body,
      );
    },
    hasIndex: (sessionId) =>
    ...
    clear: (sessionId) =>
      httpRequest(
        "POST",
        `/session/clear/${encodeURIComponent(sessionId)}`,
        {},
      ),
    updateGapThreshold: (sessionId, gapThreshold) =>
      httpRequest(
        "POST",
        `/session/update-gap-threshold/${encodeURIComponent(sessionId)}`,
        { gapThreshold },
      ),
  ...
  window.mergeApi = {
    ...
    mergeFiles: (params) => httpRequest("POST", "/merge/merge-files", params),
    clearCache: (filePath) =>
  ...
  window.chatApi = {
    checkMigration: async () => {
      ...
      return {
        ...
        pendingMigrations: Array.isArray(result.pendingMigrations)
    ...
    import: (filePath) =>
      httpRequest("POST", "/chat/import", { file_path: filePath }),
    detectFormat: (filePath) =>
      httpRequest("POST", "/chat/detect-format", { file_path: filePath }),
    importWithOptions: (filePath, formatOptions = {}) =>
      httpRequest("POST", "/chat/import-with-options", {
        file_path: filePath,
        format_options: formatOptions,
      }),
    scanMultiChatFile: (filePath) =>
      httpRequest("POST", "/chat/scan-multi-chat-file", {
        file_path: filePath,
      }),
    ...
    deleteSession: (sessionId) =>
      httpRequest("DELETE", `/chat/sessions/${encodeURIComponent(sessionId)}`),
    renameSession: (sessionId, newName) =>
      httpRequest(
        "POST",
        `/chat/sessions/${encodeURIComponent(sessionId)}/rename`,
        { new_name: newName },
      ),
    updateSessionOwnerId: (sessionId, ownerId) =>
      httpRequest(
        "POST",
        `/chat/sessions/${encodeURIComponent(sessionId)}/owner`,
        { owner_id: ownerId },
      ),
    ...
    getTimeRange: async (sessionId) => {
      const result = await httpRequest(
        "GET",
        `/chat/sessions/${encodeURIComponent(sessionId)}/time-range`,
      );
    ...
    getHourlyActivity: async (sessionId, filter) => {
      ...
      return (Array.isArray(result) ? result : []).map((item) => {
        ...
        return {
          hour: toNumber(x.period ?? x.hour, 0),
    ...
    getLaughAnalysis: (sessionId, filter, keywords = []) => {
      ...
      return httpRequest(
        "GET",
        `/chat/sessions/${encodeURIComponent(sessionId)}/laugh-analysis${toQuery(query)}`,
      );
    },
    ...
    getMembers: async (sessionId) => {
      const [membersRaw, activityRaw] = await Promise.all([
        httpRequest(
          "GET",
          `/chat/sessions/${encodeURIComponent(sessionId)}/members`,
        ),
        httpRequest(
          "GET",
          `/chat/sessions/${encodeURIComponent(sessionId)}/member-activity`,
        ).catch(() => []),
      ]);
      ...
      return (Array.isArray(membersRaw) ? membersRaw : []).map((item) =>
        normalizeMember(item, activityMap),
      );
    },
    ...
    getMemberNameHistory: async (sessionId, memberId) => {
      ...
      return (Array.isArray(result) ? result : []).map((item) => {
        ...
        return {
          ...
          endTs:
            x.endTs === null || x.endTs === undefined
              ? null
              : toNumber(x.endTs, 0),
        };
      });
    ...
    deleteMember: (sessionId, memberId) =>
      httpRequest(
        "DELETE",
        `/chat/sessions/${encodeURIComponent(sessionId)}/members/${encodeURIComponent(memberId)}`,
      ),

    pluginQuery: (sessionId, sql, params) =>
      httpRequest(
        "POST",
        `/chat/sessions/${encodeURIComponent(sessionId)}/plugin-query`,
        {
          sql,
          params,
        },
    ...
    executeSQL: (sessionId, sql) =>
      httpRequest(
        "POST",
        `/chat/sessions/${encodeURIComponent(sessionId)}/execute-sql`,
        { sql },
      ),
    generateSQL: (sessionId, prompt, options = {}) =>
      httpRequest(
        "POST",
        `/chat/sessions/${encodeURIComponent(sessionId)}/generate-sql`,
        {
          prompt,
  ...
  function normalizeAgentError(raw) {
    ...
    if (
      lower.includes("invalid session_id") ||
  ...
  function normalizeFilterResult(raw) {
    ...
    const blocks = (Array.isArray(data.blocks) ? data.blocks : []).map(
      (block) => ({
        startTs: toNumber(block.startTs, 0),
        endTs: toNumber(block.endTs, 0),
        hitCount: toNumber(block.hitCount, 0),
        messages: (Array.isArray(block.messages) ? block.messages : []).map(
          normalizeAiMessage,
        ),
      }),
    );
  ...
  window.llmApi = {
    ...
    updateConfig: (id, updates) =>
      httpRequest(
        "POST",
        `/llm/configs/${encodeURIComponent(id)}`,
        updates || {},
      ),
    deleteConfig: (id) =>
      httpRequest("DELETE", `/llm/configs/${encodeURIComponent(id)}`),
    ...
    chat: (messages, options) =>
      httpRequest("POST", "/llm/chat", { messages, options }),
  ...
  window.embeddingApi = {
    ...
    getConfig: (id) =>
      httpRequest("GET", `/embedding/configs/${encodeURIComponent(id)}`),
    ...
    updateConfig: (id, updates) =>
      httpRequest(
        "POST",
        `/embedding/configs/${encodeURIComponent(id)}`,
        updates || {},
      ),
    deleteConfig: (id) =>
      httpRequest("DELETE", `/embedding/configs/${encodeURIComponent(id)}`),
    setActiveConfig: (id) =>
      httpRequest("POST", "/embedding/active-config", { id }),
    validateConfig: (config) =>
      httpRequest("POST", "/embedding/validate-config", config),
    getVectorStoreStats: () =>
      httpRequest("GET", "/embedding/vector-store-stats"),
    clearVectorStore: () =>
      httpRequest("POST", "/embedding/clear-vector-store"),
  };
  ...
  window.aiApi = {
    ...
    semanticSearchMessages: async (
      ...
      return {
        messages: rows.map((item) => {
          ...
          return {
            ...msg,
            similarity: Number.isFinite(Number(x.similarity))
              ? Number(x.similarity)
              : 0,
          };
        }),
    ...
    filterMessagesWithContext: async (
      ...
      const result = await httpRequest(
        "POST",
        "/ai/filter-messages-with-context",
        {
          sessionId,
          keywords,
          timeFilter,
    ...
    getMultipleSessionsMessages: async (
      sessionId,
      chatSessionIds,
      page = 1,
      pageSize = 50,
    ...
    updateConversationTitle: (conversationId, title) =>
      httpRequest(
        "POST",
        `/ai/conversations/${encodeURIComponent(conversationId)}/title`,
        { title },
      ),
  ...
  window.cacheApi = {
    ...
    clear: (cacheId) =>
      httpRequest("POST", `/cache/clear/${encodeURIComponent(cacheId)}`),
    openDir: (cacheId) =>
      httpRequest("POST", `/cache/open-dir/${encodeURIComponent(cacheId)}`),
  ...
  window.networkApi = {
    ...
    saveProxyConfig: (config) =>
      httpRequest("POST", "/network/proxy-config", config || {}),
  ...
  window.nlpApi = {
    getWordFrequency: (params) =>
      httpRequest("POST", "/nlp/word-frequency", params || {}),
  ...
  window.agentApi = {
    runStream: (
      ...
      const ownerIdRaw =
        ownerInfo?.id ?? ownerInfo?.memberId ?? ownerInfo?.member_id;
      const ownerIdNum = Number(ownerIdRaw);
      const ownerId =
        Number.isFinite(ownerIdNum) && ownerIdNum > 0 ? ownerIdNum : null;
      ...
      const payload = {
        ...
        context: {
          ...
          timeFilter: rawTimeFilter
            ? {
                startTs:
                  rawTimeFilter.startTs ?? rawTimeFilter.start_ts ?? null,
                endTs: rawTimeFilter.endTs ?? rawTimeFilter.end_ts ?? null,
              }
            : null,
          maxMessagesLimit:
            context.maxMessagesLimit ?? context.max_messages_limit ?? null,
          ownerInfo: ownerInfo
            ? {
                id: ownerId,
        ...
        promptConfig: promptConfig
          ? {
              roleDefinition:
                promptConfig.roleDefinition ??
                promptConfig.role_definition ??
                "",
              responseRules:
                promptConfig.responseRules ?? promptConfig.response_rules ?? "",
            }
          : null,
        ...
        try {
          const chunks = await postSse(
            ...
            (chunk) => {
              ...
              if (
                normalizedChunk.type === "tool_start" &&
                normalizedChunk.toolName
              ) {
          ...
          for (const rawChunk of chunks) {
            ...
            if (chunk.type === "error") {
              ...
              return {
                ...
                errorMessage:
                  chunk.errorMessage || chunk.error || normalized.message,
              };
            }
          }
    ...
    abort: async (requestId) => {
      ...
      return httpRequest(
        "POST",
        `/agent/abort/${encodeURIComponent(requestId)}`,
      );
    },
    listTools: async () => {
  ...
  window.api = {
    ...
    app: {
      ...
      getAnalyticsEnabled: () =>
        httpRequest("GET", "/core/app/analytics-enabled"),
    ...
    dialog: {
      showOpenDialog: (options) =>
        httpRequest("POST", "/core/dialog/open", options),
    },
    clipboard: {
      copyImage: (dataUrl) =>
        httpRequest("POST", "/core/clipboard/copy-image", { dataUrl }),
    },
  };
  ...
  window.electron = {
    ipcRenderer: {
      send: (channel, ...args) =>
        console.log(`[Electron stub] ipcRenderer.send('${channel}')`, args),
      invoke: (channel, ...args) => {
        console.log(`[Electron stub] ipcRenderer.invoke('${channel}')`, args);
        return Promise.reject(
          new Error(`Electron IPC not available: ${channel}`),
        );
      },
      on: (channel, _listener) =>
        console.log(`[Electron stub] ipcRenderer.on('${channel}')`),
    ...
    webUtils: {
      getPathForFile: (file) => {
        ...
        return Promise.reject(
          new Error("webUtils.getPathForFile not available"),
        );
      },
    },
  };
