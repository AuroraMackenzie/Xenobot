...
async function loadData() {
  ...
  try {
    const [hourlyData, dailyData, typeData] = await Promise.all([
      ...
      window.chatApi.getMessageTypeDistribution(
        props.sessionId,
        props.timeFilter,
      ),
    ]);
...
const totalMessages = computed(() => {
  const byTypes = messageTypes.value.reduce(
...
const activeDays = computed(
  () => daily.value.filter((d) => d.messageCount > 0).length,
);
  ...
  <div class="main-content space-y-5 p-6">
    ...
    <template v-else-if="hasData">
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div
          class="rounded-xl border border-cyan-100 bg-white/80 p-4 dark:border-cyan-500/20 dark:bg-slate-900/60"
        >
          <p
        ...
        <div
          class="rounded-xl border border-amber-100 bg-white/80 p-4 dark:border-amber-500/20 dark:bg-slate-900/60"
        >
          <p
            class="text-xs font-semibold tracking-wide text-slate-500 dark:text-slate-400"
          >
          ...
          <p
            class="mt-2 text-2xl font-black text-amber-600 dark:text-amber-300"
          >
            {{ peakHour }}
          </p>
        </div>
        <div
          class="rounded-xl border border-teal-100 bg-white/80 p-4 dark:border-teal-500/20 dark:bg-slate-900/60"
        >
          <p
            class="text-xs font-semibold tracking-wide text-slate-500 dark:text-slate-400"
          >
          ...
          <p class="mt-2 text-2xl font-black text-teal-600 dark:text-teal-300">
            {{ activeDays }}
          </p>
        </div>
      </div>
