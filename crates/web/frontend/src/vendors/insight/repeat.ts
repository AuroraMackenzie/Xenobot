...
export async function queryXenoRepeatAnalysis(
  ...
  try {
    const result = (await window.chatApi.executeSQL(
      sessionId,
      sql,
    )) as SQLResult;
    ...
    const totalRepeatChains = hotContents.reduce(
      (sum, item) => sum + item.count,
      0,
    );
    const avgChainLength = hotContents.length
      ? hotContents.reduce((sum, item) => sum + item.maxChainLength, 0) /
        hotContents.length
      : 0;
