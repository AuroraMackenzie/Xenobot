...
async function loadData() {
  ...
  try {
    const result = await window.chatApi.getMemberActivity(
      props.sessionId,
      props.timeFilter,
    );
...
const totalCount = computed(() =>
  rankedMembers.value.reduce((sum, item) => sum + item.messageCount, 0),
);
...
<template>
  <div class="main-content space-y-5 p-6">
    ...
    <template v-else-if="rankedMembers.length > 0">
        ...
        <div class="space-y-2 p-4">
          ...
          >
            <div class="mb-2 flex items-center justify-between">
              ...
              <span
                class="text-sm font-bold text-cyan-600 dark:text-cyan-300"
                >{{ item.messageCount.toLocaleString() }}</span
              >
            </div>
            <div class="h-2 rounded-full bg-slate-200 dark:bg-slate-700">
              <div
                ...
                :style="{
                  width: `${Math.max(6, (item.messageCount / maxCount) * 100)}%`,
                }"
              />
            </div>
          </div>
      ...
      <div
        class="rounded-xl border border-amber-100 bg-amber-50/70 p-4 dark:border-amber-500/20 dark:bg-amber-500/8"
      >
        <p
          class="text-xs font-semibold tracking-wide text-amber-700 dark:text-amber-300"
        >
          TOP 20 MESSAGE TOTAL
        </p>
        <p class="mt-2 text-2xl font-black text-amber-600 dark:text-amber-300">
