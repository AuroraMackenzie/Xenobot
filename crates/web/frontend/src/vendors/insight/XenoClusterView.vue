...
async function loadData() {
  ...
  try {
    clusterData.value = await window.chatApi.getClusterGraph(
      props.sessionId,
      props.timeFilter,
    );
  } catch (error) {
...
const communities = computed(() =>
  Array.isArray(clusterData.value?.communities)
    ? clusterData.value.communities
    : [],
);
const hasData = computed(
  () => graphData.value.nodes.length > 0 && graphData.value.links.length > 0,
);
</script>
  ...
  <div class="main-content space-y-5 p-6">
    ...
    <template v-else-if="hasData">
      <div class="grid grid-cols-2 gap-3 lg:grid-cols-5">
        <div
          class="rounded-xl border border-cyan-100 bg-white/80 p-3 dark:border-cyan-500/20 dark:bg-slate-900/60"
        >
          <p class="text-[11px] font-semibold text-slate-500">MEMBERS</p>
          <p class="mt-1 text-xl font-black text-cyan-600 dark:text-cyan-300">
            {{ stats.totalMembers || 0 }}
          </p>
        </div>
        <div
          class="rounded-xl border border-teal-100 bg-white/80 p-3 dark:border-teal-500/20 dark:bg-slate-900/60"
        >
          <p class="text-[11px] font-semibold text-slate-500">INVOLVED</p>
          <p class="mt-1 text-xl font-black text-teal-600 dark:text-teal-300">
            {{ stats.involvedMembers || 0 }}
          </p>
        </div>
        <div
          class="rounded-xl border border-indigo-100 bg-white/80 p-3 dark:border-indigo-500/20 dark:bg-slate-900/60"
        >
          <p class="text-[11px] font-semibold text-slate-500">EDGES</p>
          <p
            class="mt-1 text-xl font-black text-indigo-600 dark:text-indigo-300"
          >
            {{ stats.edgeCount || 0 }}
          </p>
        </div>
        <div
          class="rounded-xl border border-amber-100 bg-white/80 p-3 dark:border-amber-500/20 dark:bg-slate-900/60"
        >
          <p class="text-[11px] font-semibold text-slate-500">COMMUNITIES</p>
          <p class="mt-1 text-xl font-black text-amber-600 dark:text-amber-300">
            {{ stats.communityCount || 0 }}
          </p>
        </div>
        <div
          class="rounded-xl border border-sky-100 bg-white/80 p-3 dark:border-sky-500/20 dark:bg-slate-900/60"
        >
          <p class="text-[11px] font-semibold text-slate-500">MESSAGES</p>
          <p class="mt-1 text-xl font-black text-sky-600 dark:text-sky-300">
