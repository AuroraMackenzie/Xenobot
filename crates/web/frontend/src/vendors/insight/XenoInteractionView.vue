...
async function loadData() {
  ...
  try {
    mentionData.value = await window.chatApi.getMentionAnalysis(
      props.sessionId,
      props.timeFilter,
    );
  } catch (error) {
...
const topMentioners = computed<MentionRank[]>(() =>
  (mentionData.value?.topMentioners || []).slice(0, 10),
);
const topMentioned = computed<MentionRank[]>(() =>
  (mentionData.value?.topMentioned || []).slice(0, 10),
);

const graphData = computed(() => {
  const nodeMap = new Map<
    string,
    { id: string; name: string; value: number; symbolSize: number }
  >();
...
const hasData = computed(
  () => graphData.value.nodes.length > 0 || topMentioners.value.length > 0,
);
</script>
          ...
          <ul class="space-y-2 p-4">
            ...
            >
              <div class="flex items-center gap-2">
                <span
                  class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-cyan-500 text-xs font-bold text-white"
                >
                  {{ idx + 1 }}
                </span>
                <span
              ...
              <span
                class="text-sm font-semibold text-cyan-600 dark:text-cyan-300"
                >{{ item.count }}</span
              >
            </li>
          </ul>
        ...
        <SectionCard title="Top Mentioned">
          <ul class="space-y-2 p-4">
            ...
            >
              <div class="flex items-center gap-2">
                <span
                  class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-amber-500 text-xs font-bold text-white"
                >
                  {{ idx + 1 }}
                </span>
                <span
              ...
              <span
                class="text-sm font-semibold text-amber-600 dark:text-amber-300"
                >{{ item.count }}</span
              >
            </li>
          </ul>
