  ...
  (query) => {
    if (query) {
      formData.value = {
        ...
        startDate: query.startTs
          ? dayjs.unix(query.startTs).format("YYYY-MM-DD")
          : "",
        endDate: query.endTs
          ? dayjs.unix(query.endTs).format("YYYY-MM-DD")
          : "",
      };
    }
  },
    ...
    <div class="flex flex-wrap items-center gap-3">
      ...
      <div class="min-w-0 flex flex-1 flex-wrap items-center gap-2">
        <DatePicker
          ...
          :placeholder="t('records.filter.startDate')"
          class="min-w-[10rem]"
        />
        <span class="text-xs text-gray-400">~</span>
        <DatePicker
          v-model="formData.endDate"
          :placeholder="t('records.filter.endDate')"
          class="min-w-[10rem]"
        />
      </div>
    </div>
