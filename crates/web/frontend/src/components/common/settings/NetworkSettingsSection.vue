...
const proxyModeLabel = computed(() => {
  return (
    proxyModeOptions.value.find((item) => item.value === proxyMode.value)
      ?.label ?? proxyMode.value
  );
});

const enabledFeatureCount = computed(() => {
  return Object.values(runtimeProbe.value?.features ?? {}).filter(Boolean)
    .length;
});

const publishedEndpointCount = computed(
  () => runtimeProbe.value?.endpoints.length ?? 0,
);
...
const runtimeMetaRows = computed(() => {
  ...
  return [
    ...
    },
    {
      ...
      value:
        runtimeProbe.value.bindAddr || t("settings.basic.network.unknownValue"),
    },
    {
    ...
    },
    {
      ...
      value:
        runtimeProbe.value.runtimeOs ||
        t("settings.basic.network.unknownValue"),
    },
    {
      label: t("settings.basic.network.architecture"),
      value:
        runtimeProbe.value.runtimeArch ||
        t("settings.basic.network.unknownValue"),
    },
    {
      label: "CORS",
...
const signalCards = computed(() => {
  ...
  const modulesDetail = runtimeProbe.value
    ? t("settings.basic.network.routesPublished", {
        count: publishedEndpointCount.value,
      })
  ...
  const sandboxDetail = sandboxProbe.value
    ? sandboxProbe.value.fileGatewayWritable
      ...
      : sandboxProbe.value.fileGatewayError ||
        t("settings.basic.network.sandboxRequiresReview")
...
async function saveProxyConfig() {
  ...
  try {
    ...
    if (!result.success) {
      proxyUrlError.value =
        result.error || t("settings.basic.network.saveFailed");
    }
  } catch (error) {
...
async function testProxyConnection() {
  ...
  try {
    const result = await window.networkApi.testProxyConnection(
      proxyUrl.value.trim(),
    );
...
async function refreshRuntimeSurface() {
  ...
  try {
    ...
    const service = asString(
      statusBody.service,
      asString(indexBody.service, "xenobot-api"),
    );
...
async function refreshSandboxDoctor() {
  ...
  try {
    const result = await window.networkApi.getSandboxDoctor(
      sandboxGatewayDir.value.trim(),
    );
...
async function checkMcpConnectivity() {
  ...
  try {
    ...
    const httpToolCount = Array.isArray(httpTools.tools)
      ? httpTools.tools.length
      : 0;
    const rpcToolsResult = (rpcTools.result || {}) as { tools?: unknown[] };
    const rpcToolCount = Array.isArray(rpcToolsResult.tools)
      ? rpcToolsResult.tools.length
      : 0;
...
async function loadMcpIntegrationCatalog() {
  ...
  try {
    const mcpApi = window.mcpApi;
    if (!mcpApi) {
      integrationPresetError.value = t(
        "settings.basic.network.mcpRuntimeMissing",
      );
      return;
    }
    ...
    const rawItems = Array.isArray(result.integrations)
      ? result.integrations
      : [];
    ...
    if (
      ...
      !integrationTargets.value.some(
        (item) => item.id === selectedIntegrationTarget.value,
      )
    ) {
      selectedIntegrationTarget.value =
        integrationTargets.value[0]?.id || "pencil";
    }
  } catch (error) {
    integrationPresetError.value =
      error instanceof Error ? error.message : String(error);
...
async function loadSelectedIntegrationPreset() {
  ...
  try {
    ...
    const baseUrl = normalizeEndpointBase(mcpBaseUrl.value);
    const result = await mcpApi.getIntegrationPreset(
      selectedIntegrationTarget.value,
      baseUrl,
    );
    if (!result.success) {
...
<template>
  <div class="network-surface space-y-3">
    ...
    <section
      class="xeno-panel rounded-2xl border border-gray-200/80 p-4 dark:border-gray-700/80"
    >
      ...
      <div v-if="proxyMode === 'manual'" class="mt-4 space-y-3">
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          >
            ...
            {{
              isTestingProxy
                ? t("settings.basic.network.testing")
                : t("settings.basic.network.testConnection")
            }}
          </UButton>
      ...
      <Transition name="xeno-fade">
        ...
        >
          ...
          <span class="status-note__meta">
            {{ t("settings.basic.network.lastChecked") }}
            {{ runtimeProbe.checkedAt }}
          </span>
        </div>
      </Transition>

      <div
        class="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]"
      >
        <div class="meta-grid">
          <div class="meta-cell">
            <p class="meta-label">Service</p>
            <p class="meta-value">
              {{ runtimeProbe?.service || "xenobot-api" }}
            </p>
          </div>
          <div class="meta-cell">
        ...
        <div class="space-y-3">
          <div class="meta-group">
            <div class="flex items-center justify-between gap-2">
              <p
                class="text-xs font-medium uppercase tracking-[0.22em] text-gray-500 dark:text-gray-400"
              >
            ...
            <div class="chip-grid mt-2">
              ...
              <span
                v-if="!runtimeProbe?.endpoints?.length"
          ...
          <div class="meta-group">
            ...
            <div class="chip-grid mt-2">
              ...
              <span
                v-if="!runtimeFeatureEntries.length"
                class="chip chip--muted"
              >
                {{ t("settings.basic.network.runtimeHint") }}
              </span>
      ...
      >
        ...
        <Transition name="xeno-fade">
          ...
          >
            ...
            />
            <span>
              {{ t("settings.basic.network.sandboxRecommended") }}:
              {{ sandboxProbe.recommendedMode }}
            </span>
            <span class="status-note__meta">
              {{ t("settings.basic.network.lastChecked") }}
              {{ sandboxProbe.checkedAt }}
            </span>
          </div>
        </Transition>
        ...
        <div v-if="sandboxProbe" class="meta-grid mt-3">
          <div class="meta-cell">
            <p class="meta-label">
              {{ t("settings.basic.network.tcpCapability") }}
            </p>
            <p class="meta-value">
              {{
                boolToken(
          ...
          <div class="meta-cell">
            <p class="meta-label">
              {{ t("settings.basic.network.udsCapability") }}
            </p>
            <p class="meta-value">
              {{
          ...
          <div class="meta-cell">
            <p class="meta-label">
              {{ t("settings.basic.network.fileGatewayCapability") }}
            </p>
            <p class="meta-value">
              {{
                boolToken(
        ...
        <div
          v-if="sandboxProbe"
          class="mt-3 grid gap-3 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)]"
        >
          <div class="space-y-3">
            <div class="meta-group">
              <p
              ...
              <div
                class="mt-2 space-y-2 text-xs text-gray-600 dark:text-gray-300"
              >
                <p>
                  ...
                  >
                  {{
                    sandboxProbe.tcpError ||
                    t("settings.basic.network.noIssueDetected")
                  }}
                </p>
                <p>
                  <span class="font-semibold text-gray-800 dark:text-gray-100"
                  ...
                  >
                  {{
                    sandboxProbe.udsError ||
                    sandboxProbe.udsPath ||
                    t("settings.basic.network.noIssueDetected")
                  }}
                </p>
                <p>
                  ...
                  >
                  {{
                    sandboxProbe.fileGatewayError ||
                    sandboxProbe.fileGatewayDir ||
                    t("settings.basic.network.noIssueDetected")
                  }}
                </p>
              </div>
          ...
          <div class="result-panel">
            <p
              class="mb-2 text-xs font-medium uppercase tracking-[0.22em] text-gray-500 dark:text-gray-400"
            >
              {{ t("settings.basic.network.sandboxCommand") }}
            </p>
            <pre
              class="max-h-72 overflow-auto whitespace-pre-wrap break-all text-[11px] leading-relaxed text-gray-700 dark:text-gray-200"
              >{{
                sandboxProbe.recommendedCommand ||
                t("settings.basic.network.runtimeHint")
              }}</pre
            >
    ...
    >
      <div class="flex flex-wrap items-start justify-between gap-3">
        ...
        >
          ...
          {{
            isCheckingMcp
              ? t("settings.basic.network.checkingMcp")
              : t("settings.basic.network.checkMcp")
          }}
        </UButton>
      </div>
      ...
      <div v-if="mcpCheckResult" class="meta-grid mt-3">
        <div class="meta-cell">
          <p class="meta-label">
            {{ t("settings.basic.network.mcpHttpStatus") }}
          </p>
          <p class="meta-value">{{ mcpCheckResult.httpStatus }}</p>
        </div>
        <div class="meta-cell">
          <p class="meta-label">
            {{ t("settings.basic.network.mcpHttpTools") }}
          </p>
          <p class="meta-value">{{ mcpCheckResult.httpToolCount }}</p>
        </div>
        <div class="meta-cell">
          <p class="meta-label">
            {{ t("settings.basic.network.mcpRpcTools") }}
          </p>
      ...
      >
        <div class="flex flex-wrap items-start justify-between gap-3">
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <UButton
              ...
              :disabled="
                isLoadingIntegrationPreset || !selectedIntegrationTarget
              "
              color="neutral"
              variant="soft"
              size="sm"
        ...
        <div
          class="mt-3 grid gap-3 md:grid-cols-[minmax(0,220px)_minmax(0,1fr)]"
        >
          <div>
            <label
              class="mb-1.5 block text-xs font-medium text-gray-700 dark:text-gray-300"
            ...
            <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
              {{
                integrationTargets.find(
                  (item) => item.id === selectedIntegrationTarget,
                )?.description ||
                t("settings.basic.network.integrationPresetDesc")
              }}
            </p>
          ...
          <div>
            ...
            <div v-if="integrationPresetText" class="result-panel">
              ...
              <pre
                class="max-h-72 overflow-auto whitespace-pre-wrap break-all text-[11px] leading-relaxed text-gray-700 dark:text-gray-200"
                >{{ integrationPresetText }}</pre
              >
            </div>
            <div v-else-if="!integrationPresetError" class="meta-group">
              <p class="text-xs text-gray-500 dark:text-gray-400">
    ...
    >
      ...
      <Transition name="xeno-fade">
        <div v-if="mcpToolError" class="status-note status-note--warning mt-3">
          <UIcon name="i-heroicons-exclamation-triangle" class="h-4 w-4" />
          <span>{{ mcpToolError }}</span>
        </div>
      </Transition>

      <div v-if="mcpToolResultText" class="result-panel mt-3">
        <p
          class="mb-2 text-xs font-medium uppercase tracking-[0.22em] text-gray-500 dark:text-gray-400"
        >
