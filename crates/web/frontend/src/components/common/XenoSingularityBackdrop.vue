...
function resizeCanvas() {
  ...
  singularity.radius = Math.max(
    78,
    Math.min(132, Math.min(width, height) * 0.102),
  );
...
function lensPoint(x: number, y: number): LensSample {
  ...
  const ringFactor = Math.exp(
    -Math.pow((distance - ringRadius) / ringSigma, 2),
  );
  const gravitationalTerm =
    (singularity.radius * singularity.radius * 0.09) /
    (distance + singularity.radius * 0.6);
...
function drawStars(timeSeconds: number) {
  ...
  for (const star of stars) {
    const driftX =
      Math.sin(timeSeconds * 0.035 * star.drift + star.twinkle) *
      18 *
      star.depth *
      motionScale;
    const driftY =
      Math.cos(timeSeconds * 0.028 * star.drift + star.twinkle * 0.7) *
      12 *
      star.depth *
      motionScale;
    ...
    const pulse =
      0.5 +
      0.5 * Math.sin(timeSeconds * (0.8 + star.depth * 0.7) + star.twinkle);
    const alphaScale = darkMode ? 0.82 : 0.34;
    const radius =
      star.radius * (0.8 + pulse * 0.28) * (0.94 + sample.ringFactor * 0.28);
    const color = starColor(sample, star, pulse);

    if (sample.ringFactor > 0.12) {
      ctx.beginPath();
      ctx.moveTo(
        sample.x - sample.tangentX * radius * 2.6,
        sample.y - sample.tangentY * radius * 2.6,
      );
      ctx.lineTo(
        sample.x + sample.tangentX * radius * 2.6,
        sample.y + sample.tangentY * radius * 2.6,
      ...
      ctx.strokeStyle = color.replace(
        /[\d.]+\)$/,
        `${(0.18 + sample.ringFactor * 0.18) * alphaScale})`,
      );
    ...
    halo.addColorStop(
      ...
      color.replace(
        /[\d.]+\)$/,
        `${(0.34 + sample.ringFactor * 0.26) * alphaScale})`,
      ),
    );
  ...
  const vignette = ctx.createRadialGradient(
    centerX,
    centerY,
    singularity.radius * 1.4,
    centerX,
    centerY,
    singularity.radius * 6.2,
  );
  vignette.addColorStop(
    0,
    darkMode ? "rgba(0, 0, 0, 0.02)" : "rgba(255, 255, 255, 0.01)",
  );
  vignette.addColorStop(
    1,
    darkMode ? "rgba(0, 0, 0, 0.24)" : "rgba(255, 255, 255, 0.08)",
  );
...
function drawAccretionDisc(timeSeconds: number) {
  ...
  for (let pass = 0; pass < 2; pass += 1) {
    ...
    for (let i = 0; i < steps; i += 1) {
      ...
      const radius =
        innerRadius + (outerRadius - innerRadius) * (0.16 + 0.84 * radialNoise);
      ...
      const lensLift = !frontPass
        ? Math.exp(-Math.pow(x / (singularity.radius * 2.6), 2)) *
          singularity.radius *
          0.56
        : 0;
      y -= lensLift;
...
function drawSingularityCore() {
  ...
  ring.addColorStop(
    0.58,
    darkMode ? "rgba(4, 10, 18, 0.94)" : "rgba(8, 16, 26, 0.8)",
  );
  ring.addColorStop(
    0.74,
    darkMode ? "rgba(55, 200, 255, 0.2)" : "rgba(55, 200, 255, 0.08)",
  );
  ring.addColorStop(
    0.86,
    darkMode ? "rgba(255, 190, 110, 0.12)" : "rgba(255, 190, 110, 0.05)",
  );
  ...
  ctx.strokeStyle = darkMode
    ? "rgba(120, 226, 255, 0.42)"
    : "rgba(90, 180, 220, 0.18)";
  ctx.lineWidth = 2.2;
  ctx.beginPath();
  ctx.ellipse(
    centerX,
    centerY - singularity.radius * 0.05,
    photonRing,
    photonRing * 0.86,
    -0.12,
    0,
...
<template>
  <canvas
    ref="canvasRef"
    class="xeno-singularity-backdrop"
    aria-hidden="true"
  />
</template>
