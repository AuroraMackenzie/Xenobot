...
async function checkAndAutoOpen() {
  ...
  try {
    ...
    console.error(
      "[SessionIndexModal] Failed to inspect session index:",
      error,
    );
  } finally {
...
async function refreshStatus() {
  ...
  try {
    ...
    console.error(
      "[SessionIndexModal] Failed to refresh session index status:",
      error,
    );
  } finally {
    isLoading.value = false;
...
async function generateSessionIndex() {
  ...
  try {
    ...
    const count = await window.sessionApi.generate(
      props.sessionId,
      gapThreshold,
    );
    hasIndex.value = true;
    ...
    console.error(
      "[SessionIndexModal] Failed to generate session index:",
      error,
    );
  } finally {
      ...
      <div class="xeno-session-index-card p-6">
        <div class="mb-4 flex items-center justify-between">
          <div class="flex items-center gap-2">
            <div
              class="xeno-session-index-icon flex h-10 w-10 items-center justify-center rounded-full"
            >
              <UIcon
        ...
        <template v-else>
          <div v-if="!hasIndex" class="space-y-4">
            <div class="xeno-session-index-alert rounded-2xl p-4">
          ...
          <div v-else class="space-y-4">
            <div class="xeno-session-index-success rounded-2xl p-4">
              <div class="flex gap-3">
                <UIcon
                ...
                <div>
                  ...
                  <p class="mt-1 text-sm text-green-700 dark:text-green-300">
                    {{
                      t("records.sessionIndex.sessionCount", {
                        count: sessionCount,
                      })
                    }}
                  </p>
        ...
        <div class="mt-6 flex justify-end gap-2">
          ...
          >
            <UIcon
              v-if="!isGenerating"
              :name="
                hasIndex ? 'i-heroicons-arrow-path' : 'i-heroicons-sparkles'
              "
              class="mr-1 h-4 w-4"
            />
            {{
