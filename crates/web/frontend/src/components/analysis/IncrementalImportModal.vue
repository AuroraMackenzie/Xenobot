...
async function analyzeFile() {
  ...
  try {
    const result = await window.chatApi.analyzeIncrementalImport(
      props.sessionId,
...
async function executeImport() {
  ...
  try {
    ...
    const result = await window.chatApi.incrementalImport(
      props.sessionId,
      selectedFile.value.path,
    );
    unsubscribe();
...
<template>
  <UModal
    v-model:open="isOpen"
    :title="t('analysis.incremental.title')"
      ...
      <div class="xeno-incremental-shell min-h-[240px]">
            ...
            <template #default="{ isDragOver }">
              ...
              >
                <UIcon
                  ...
                  class="mb-3 h-10 w-10 text-sky-300/90"
                />
                <p class="text-center text-sm text-gray-600 dark:text-gray-400">
                  {{
                    isDragOver
                      ? t("home.import.dropHint")
                      : t("analysis.incremental.dropHint")
                  }}
                </p>
              </div>
            </template>
        ...
        >
          <UIcon
            ...
            class="mb-4 h-10 w-10 animate-spin text-sky-400"
          />
          <p class="text-gray-600 dark:text-gray-400">
            {{ t("analysis.incremental.analyzing") }}
          </p>
          <p class="mt-2 break-all text-center text-sm text-gray-500">
            {{ selectedFile?.name }}
          </p>
        </div>
        ...
        <div v-else-if="stage === 'preview' && analyzeResult" class="space-y-6">
          <div class="xeno-incremental-panel rounded-2xl p-4">
            <p
              class="mb-2 break-all text-sm font-medium text-gray-700 dark:text-gray-300"
            >
              {{ selectedFile?.name }}
            </p>
            <div class="grid grid-cols-3 gap-4 text-center">
              <div>
                ...
                <p class="text-xs text-gray-500">
                  {{ t("analysis.incremental.totalInFile") }}
                </p>
              </div>
              <div>
                <p
                ...
                <p class="text-xs text-gray-500">
                  {{ t("analysis.incremental.newMessages") }}
                </p>
              </div>
              <div>
                ...
                <p class="text-xs text-gray-500">
                  {{ t("analysis.incremental.duplicates") }}
                </p>
              </div>
            </div>
          </div>
          ...
          <p
            v-if="analyzeResult.newMessageCount === 0"
            class="text-center text-sm text-amber-600 dark:text-amber-400"
          >
            <UIcon
        ...
        >
          ...
          />
          <p class="text-gray-600 dark:text-gray-400">
            {{ t("analysis.incremental.importing") }}
          </p>
        ...
        <div
          v-else-if="stage === 'done' && importResult"
          class="flex flex-col items-center justify-center py-10"
        >
          <UIcon
          ...
          <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
            {{
              t("analysis.incremental.successDetail", {
                count: importResult.newMessageCount,
              })
            }}
          </p>
        </div>
        ...
        >
          ...
          <p
            class="mt-2 break-words text-center text-sm text-red-600 dark:text-red-400"
          >
            {{ errorMessage }}
          </p>
        </div>
    ...
    <template #footer>
      <div class="flex w-full justify-end gap-2">
        ...
        <template v-else-if="stage === 'preview'">
          ...
          >
            {{
              t("analysis.incremental.import", {
                count: analyzeResult?.newMessageCount || 0,
              })
            }}
          </UButton>
        </template>
