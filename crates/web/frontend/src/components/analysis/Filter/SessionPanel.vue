...
const virtualizer = useVirtualizer(
  computed(() => ({
    ...
    estimateSize: (index) =>
      flatItems.value[index]?.type === "date" ? 36 : 56,
    overscan: 10,
  })),
);
...
function getSessionIdsForDate(date: string): number[] {
  return flatItems.value
    .filter((item) => item.type === "session" && item.date === date)
...
function isDateFullySelected(date: string): boolean {
  const sessionIds = getSessionIdsForDate(date);
  return (
    sessionIds.length > 0 && sessionIds.every((id) => selectedSet.value.has(id))
  );
}
...
<template>
  <div class="xeno-session-shell flex h-full min-h-0 flex-col p-4">
    <div
      ...
      class="flex-none mb-2 text-sm text-primary-500"
    >
    ...
    <div class="flex-1 min-h-0">
      ...
      <div
        ...
        class="xeno-session-empty flex items-center justify-center h-full text-gray-500 text-sm"
      >
      ...
      <div
        ...
        class="xeno-session-scroll h-full overflow-y-auto rounded-2xl"
      >
        <div
          :style="{
            ...
            >
              ...
              <div class="flex items-center gap-2">
                <span class="text-xs text-gray-500">
                  {{
                    t("analysis.filter.historySessionCount", {
                      count: flatItems[virtualRow.index].sessionCount,
                    })
                  }}
                </span>
                <input
                  type="checkbox"
                  :checked="
                    isDateFullySelected(flatItems[virtualRow.index].date!)
                  "
            ...
            <label
              ...
              :class="{
                'border-primary-300 bg-primary-50 dark:border-primary-600 dark:bg-primary-900/20':
                  isSelected(flatItems[virtualRow.index].session!.id),
              }"
            >
              <input
                type="checkbox"
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-2 text-xs text-gray-500">
                  <span>{{
                    t("analysis.filter.sessionMessageCount", {
                      count: flatItems[virtualRow.index].session!.messageCount,
                    })
                  }}</span>
                  <span>·</span>
                  <span>
