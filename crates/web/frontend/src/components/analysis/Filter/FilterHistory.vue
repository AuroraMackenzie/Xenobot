...
function formatSummary(item: FilterHistoryItem): string {
  if (item.mode === 'condition') {
    ...
    if (item.conditionFilter?.keywords.length) {
      parts.push(`${t('analysis.filter.historyKeywordPrefix')}: ${item.conditionFilter.keywords.join(', ')}`)
    }
...
defineExpose({
  saveCondition(condition: Omit<FilterHistoryItem, 'id' | 'sessionId' | 'createdAt' | 'name'>) {
    const newItem: FilterHistoryItem = {
      ...
      createdAt: Date.now(),
      name: t('analysis.filter.historyAutoName', { index: historyList.value.length + 1 }),
      ...condition,
    }
...
<template>
  <UModal v-model:open="open" :ui="{ overlay: 'z-[10001]', content: 'z-[10001] max-w-3xl' }">
    <template #content>
      <UCard class="xeno-filter-history-card">
        ...
        <div class="xeno-filter-history-scroll max-h-96 overflow-y-auto rounded-2xl">
          <div v-if="historyList.length === 0" class="xeno-filter-history-empty py-8 text-center text-gray-500">
              ...
              <div class="flex items-start justify-between gap-2">
                <div class="flex-1 min-w-0">
                  ...
                  <div v-else class="flex items-center gap-2 mb-1">
                    <span class="break-words font-medium text-gray-900 dark:text-white">{{ item.name }}</span>
                    <UBadge :color="item.mode === 'condition' ? 'primary' : 'green'" size="xs">
                      {{ item.mode === 'condition' ? t('analysis.filter.historyModeCondition') : t('analysis.filter.historyModeSession') }}
                    </UBadge>
