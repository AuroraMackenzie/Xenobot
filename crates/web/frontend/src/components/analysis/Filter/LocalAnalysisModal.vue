...
const presetOptions = [
  {
    id: "summary",
    label: "总结对话要点",
    prompt: "请总结这段对话的主要内容和关键要点。",
  },
  {
    id: "sentiment",
    label: "情感分析",
    prompt: "请分析这段对话中参与者的情感变化和整体氛围。",
  },
  {
    id: "topics",
    label: "话题提取",
    prompt: "请提取这段对话中讨论的主要话题，并简要说明每个话题的内容。",
  },
  {
    id: "insights",
    label: "洞察分析",
    prompt:
      "请对这段对话进行深度分析，包括参与者的关系、互动模式、潜在问题等。",
  },
];
    ...
    const { requestId, promise } = window.agentApi.runStream(
      ...
      (chunk) => {
        if (chunk.type === "content" && chunk.content) {
          ...
          analysisError.value =
            chunk.error || t("analysis.filter.analysisRuntimeError");
        }
      },
      [],
...
async function copyResult() {
  try {
    ...
    console.error(
      "[LocalAnalysisModal] Failed to copy analysis result:",
      error,
    );
  }
}
...
watch(open, (val) => {
  if (!val) {
    ...
    editablePresetPrompt.value =
      presetOptions.find((p) => p.id === selectedPreset.value)?.prompt || "";
  }
});
</script>
      ...
      <UCard class="xeno-local-analysis-card">
        <template #header>
          <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold">
              {{ t("analysis.filter.localAnalysisTitle") }}
            </h3>
            <UButton
              variant="ghost"
        ...
        <div class="space-y-4">
          <div class="xeno-local-analysis-panel rounded-xl p-3 text-sm">
            ...
            >
              <span
                >{{ filterResult?.blocks.length || 0 }}
                {{ t("analysis.filter.stats.blocks") }}</span
              >
              <span
                >{{ filterResult?.stats.totalMessages || 0 }}
                {{ t("analysis.filter.stats.messages") }}</span
              >
              <span
                >{{ filterResult?.stats.totalChars.toLocaleString() || 0 }}
                {{ t("analysis.filter.stats.chars") }}</span
              >
            </div>
          </div>
            ...
            <div class="flex items-start gap-2">
              ...
              <div class="text-sm">
                ...
                <p class="text-red-600 dark:text-red-500 mt-1">
                  {{
                    t("analysis.filter.dataTooLargeThreshold", {
                      count: DATA_TOO_LARGE_THRESHOLD,
                    })
                  }}
                </p>
              </div>
          ...
          <div
            class="xeno-local-analysis-mode flex w-fit items-center gap-1 rounded-lg p-1"
          >
            <button
          ...
          <div class="flex justify-end gap-2">
            ...
            <UButton
              ...
              :disabled="
                isAnalyzing ||
                isDataTooLarge ||
                (analysisMode === 'custom' && !customPrompt.trim())
              "
            ...
            >
              ...
              {{
                isAnalyzing
                  ? t("analysis.filter.analyzing")
                  : t("analysis.filter.startAnalysis")
              }}
            </UButton>
          </div>

          <div
            v-if="analysisResult || analysisError"
