...
import type {
  LaughAnalysis,
  KeywordTemplate as BaseKeywordTemplate,
} from "@/types/analysis";
...
const KEYWORD_COLORS = [
  {
    ...
    wrapBg: "bg-amber-50 dark:bg-amber-900/20",
  },
  {
    bg: "bg-pink-400",
    text: "text-pink-700",
  ...
  },
  {
    ...
    text: "text-blue-700",
    badge: "blue" as const,
    wrapBg: "bg-blue-50 dark:bg-blue-900/20",
  },
  {
    bg: "bg-green-400",
    text: "text-green-700",
    badge: "green" as const,
    wrapBg: "bg-green-50 dark:bg-green-900/20",
  },
  {
    bg: "bg-purple-400",
  ...
  },
  {
    ...
    text: "text-red-700",
    badge: "red" as const,
    wrapBg: "bg-red-50 dark:bg-red-900/20",
  },
  {
    bg: "bg-cyan-400",
    text: "text-cyan-700",
    badge: "cyan" as const,
    wrapBg: "bg-cyan-50 dark:bg-cyan-900/20",
  },
  {
    bg: "bg-orange-400",
...
const allTemplates = computed<KeywordTemplate[]>(() => {
  ...
  // English engineering note.
  const activePresets = PRESET_TEMPLATES.value.filter(
    (tpl) => !promptStore.deletedPresetTemplateIds.includes(tpl.id),
  );
...
const currentTemplateName = computed(() => {
  const template = allTemplates.value.find(
...
const modalTitle = computed(() =>
  isEditMode.value
    ? t("quotes.keywords.modal.editTitle")
    : t("quotes.keywords.modal.createTitle"),
);
...
function isPresetTemplate(templateId: string): boolean {
  return PRESET_TEMPLATE_IDS.includes(
    templateId as (typeof PRESET_TEMPLATE_IDS)[number],
  );
}
...
async function loadAnalysis() {
  ...
  try {
    analysis.value = await window.chatApi.getLaughAnalysis(
      props.sessionId,
      props.timeFilter,
      [...currentKeywords.value],
    );
    ...
    >
      ...
      <template #config>
        ...
        <div class="border-b border-gray-100 p-4 dark:border-gray-800">
          ...
          <div class="mb-3 flex flex-wrap items-center gap-2">
            <span class="text-xs text-gray-500 dark:text-gray-400">{{
              t("quotes.keywords.templateLabel")
            }}</span>
            <UContextMenu
                ...
                <div class="p-4">
                  ...
                  <div class="space-y-3">
                    <div>
                      ...
                      <UInput
                        ...
                        :placeholder="
                          t('quotes.keywords.modal.templateNamePlaceholder')
                        "
                      />
                    </div>
                    <div>
                      <label class="mb-1 block text-xs text-gray-500">{{
                        t("quotes.keywords.modal.keywords")
                      }}</label>
                      <div class="flex flex-wrap items-center gap-2">
                        ...
                        <UInput
                          ...
                          :placeholder="
                            t('quotes.keywords.modal.keywordPlaceholder')
                          "
                          class="w-full"
                  ...
                  <div class="mt-4 flex justify-end gap-2">
                    ...
                    <UButton
                      color="primary"
                      :disabled="
                        !templateName.trim() || templateKeywords.length === 0
                      "
                      @click="saveTemplate"
                    >
                      {{
                        isEditMode
                          ? t("quotes.keywords.modal.update")
                          : t("common.save")
                      }}
                    </UButton>
                  </div>
          ...
          <div class="mt-1.5 text-xs text-gray-400">
            {{ t("quotes.keywords.templateHint") }}
          </div>
        </div>
          ...
          <div class="flex flex-wrap gap-2">
            ...
            >
              <span
                ...
                :class="getKeywordColor(item.type).bg"
              />
              <span
                class="font-medium"
                :class="getKeywordColor(item.type).text"
                >{{ item.type }}</span
              >
              <span class="text-xs text-gray-500">{{
                t("quotes.keywords.times", { count: item.count })
              }}</span>
              <UBadge
                :color="getKeywordColor(item.type).badge"
                variant="soft"
                size="xs"
                >{{ item.percentage }}%</UBadge
              >
            </div>
          </div>
        </div>
        ...
        <LoadingState
          ...
          :text="t('quotes.keywords.loading')"
        />
      </template>
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex shrink-0 items-baseline gap-2">
            ...
            <span class="text-sm text-gray-500">
              {{
                t("quotes.keywords.timesWithPercent", {
                  count: member.value,
