...
import { queryXenoRepeatAnalysis, type XenoRepeatAnalysis } from '@/vendors/insight'
...
async function loadRepeatAnalysis() {
  ...
  try {
    repeatAnalysis.value = await queryXenoRepeatAnalysis(props.sessionId, props.timeFilter)
  } catch (error) {
...
.xeno-quotes-panel {
  ...
  background:
    radial-gradient(circle at top right, rgba(248, 113, 113, 0.09), transparent 24%),
