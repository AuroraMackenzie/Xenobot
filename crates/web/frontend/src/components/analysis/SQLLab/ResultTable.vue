...
function isValidTimestamp(value: unknown): boolean {
  ...
  return (
    ...
    (value >= MIN_MILLIS && value <= MAX_MILLIS)
  );
}
...
// English engineering note.
const MESSAGE_ID_COLUMN_PATTERNS = [
  /^id$/i,
  /^message_id$/i,
  /^msg_id$/i,
  /^msgid$/i,
...
function getMessageIdColumnIndex(columns: string[]): number {
  return columns.findIndex((col) =>
    MESSAGE_ID_COLUMN_PATTERNS.some((pattern) => pattern.test(col)),
  );
}
...
// English engineering note.
const showReadableTime = ref(
  localStorage.getItem("sql-lab-readable-time") !== "false",
);
...
function getColumnLabelLocal(columnName: string): string | null {
  ...
  // English engineering note.
  const localeLabels =
    COLUMN_LABELS[locale.value as "zh-CN" | "en-US"] || COLUMN_LABELS["zh-CN"];
...
async function exportResult() {
  ...
  try {
    const format = (aiGlobalSettings.value.sqlExportFormat ??
      "csv") as SQLExportFormat;
    ...
    console.error("[ResultTable] Failed to export SQL result:", err);
    toast.add({
      title: t("common.exportFailed"),
...
function buildResultSummary(): string {
  ...
  const dataRows = rows.map((row) =>
    row
      .map((cell) => (cell === null ? "NULL" : String(cell).slice(0, 50)))
      .join(" | "),
  );
  ...
  if (props.result.rows.length > maxRows) {
    resultText += `\n\n${t("sqlLab.summary.previewRows", { maxRows, totalRows: props.result.rowCount })}`;
  }
...
async function generateSummary() {
  ...
  try {
    ...
    let contextInfo = "";
    if (props.prompt) {
      contextInfo = `${t("sqlLab.summary.queryIntent", { prompt: props.prompt })}\n\n`;
    }
    if (props.sql) {
      contextInfo += `${t("sqlLab.summary.executedSql")}\n\`\`\`sql\n${props.sql}\n\`\`\`\n\n`;
    }

    const prompt = t("sqlLab.summary.userPrompt", {
      contextInfo,
      rowCount: props.result?.rowCount || 0,
      resultSummary,
    });
    ...
    const result = await window.llmApi.chatStream(
      [
        {
          role: "system",
          content: t("sqlLab.summary.systemPrompt"),
        },
...
<template>
  <div class="flex flex-1 flex-col overflow-hidden">
    ...
    <div
      v-if="error"
      class="border-b border-red-200 bg-red-50 p-4 dark:border-red-900 dark:bg-red-950"
    >
      <div class="flex items-start gap-2">
        ...
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-red-800 dark:text-red-200">
            {{ t("ai.sqlLab.result.queryError") }}
          </p>
          <p
            class="mt-1 break-all font-mono text-xs text-red-600 dark:text-red-400"
          >
            {{ error }}
          </p>
        </div>
    ...
    >
      ...
      >
        ...
        <label class="flex cursor-pointer items-center gap-1.5">
          <UCheckbox
            :model-value="showReadableTime"
            size="xs"
            @update:model-value="toggleReadableTime"
          />
      ...
      <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        <thead class="sticky top-0 bg-gray-100 dark:bg-gray-800">
          <tr>
              ...
              <div class="flex items-center gap-1">
                <div class="flex flex-col">
                  <span class="text-gray-700 dark:text-gray-300">{{
                    getColumnLabelLocal(column) || column
                  }}</span>
                  <span
                    v-if="getColumnLabelLocal(column)"
            ...
            <!-- English UI note -->
            <th
              v-if="showViewMessageButton"
              class="sticky right-0 w-12 bg-gray-100 px-2 py-2 dark:bg-gray-800"
            ></th>
          </tr>
        </thead>
        <tbody
        ...
        >
          ...
          >
            ...
            <td
              v-if="showViewMessageButton"
              class="sticky right-0 w-12 bg-white px-2 py-2 dark:bg-gray-900"
            >
              <UButton
    ...
    >
      ...
      <div class="flex items-center gap-1">
        ...
        <UButton
          ...
          @click="currentPage--"
        />
        <span class="mx-2 text-xs text-gray-600 dark:text-gray-400"
    ...
    >
      <div class="text-center">
        ...
        <p class="mt-1 text-xs text-gray-400">
          {{ t("ai.sqlLab.result.initialHint") }}
        </p>
      </div>
    </div>
    ...
    <UModal
      ...
      :title="t('ai.sqlLab.result.summaryTitle')"
      description="Review the generated natural-language summary for the current SQL result."
    >
      <template #content>
        <div class="max-h-[70vh] overflow-hidden p-6">
          ...
          <!-- English UI note -->
          <div
            v-if="isSummarizing && !streamingContent"
            class="flex items-center justify-center py-8"
          >
            <UIcon
              name="i-heroicons-arrow-path"
              class="h-6 w-6 animate-spin text-pink-500"
            />
            <span class="ml-2 text-sm text-gray-500">{{
              t("ai.sqlLab.result.analyzing")
            }}</span>
          </div>

          <!-- English UI note -->
          <div
            v-else-if="streamingContent || summaryContent"
          ...
          <div
            v-if="summaryError"
            class="rounded-lg bg-red-50 p-4 dark:bg-red-950"
          >
          ...
          <div class="mt-4 flex justify-end gap-2">
            <UButton
              v-if="!isSummarizing && summaryContent"
              variant="outline"
            ...
            <UButton variant="ghost" @click="closeSummaryModal">{{
              t("common.close")
            }}</UButton>
          </div>
        </div>
