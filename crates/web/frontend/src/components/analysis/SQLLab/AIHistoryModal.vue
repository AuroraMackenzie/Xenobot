...
function formatHistoryTime(timestamp: number): string {
  ...
  if (diff < 3600000)
    return t("ai.sqlLab.history.minutesAgo", {
      count: Math.floor(diff / 60000),
    });
  if (diff < 86400000)
    return t("ai.sqlLab.history.hoursAgo", {
      count: Math.floor(diff / 3600000),
    });
  if (date.toDateString() === now.toDateString())
    return t("ai.sqlLab.history.today");
    ...
    <template #content>
      <div class="max-h-[70vh] overflow-hidden p-6">
        ...
        <div
          ...
          class="max-h-[50vh] space-y-3 overflow-y-auto pr-2"
        >
          <div
