...
const props = defineProps<{
  ...
  timestamp: number;
  isStreaming?: boolean;
  /** Structured blocks returned by the assistant renderer, when available. */
  contentBlocks?: ContentBlock[];
  /** Whether the capture button should be shown for this message bubble. */
...
function getThinkLabel(tag: string): string {
  ...
  if (normalized === "analysis")
    return t("ai.chat.message.think.labels.analysis");
  if (normalized === "reasoning")
    return t("ai.chat.message.think.labels.reasoning");
  if (normalized === "reflection")
    return t("ai.chat.message.think.labels.reflection");
  if (
    normalized === "think" ||
    normalized === "thought" ||
    normalized === "thinking"
...
function formatTimeParams(params: Record<string, unknown>): string {
  ...
  if (params.year) {
    if (locale.value === "zh-CN") {
      ...
      if (params.month) {
        result =
          monthNames[(params.month as number) - 1] || String(params.month);
        if (params.day) {
          result += ` ${params.day}`;
          if (params.hour !== undefined) {
...
function formatToolParams(tool: ToolBlockContent): string {
  ...
  if (name === "search_messages" || name === "semantic_search") {
    ...
    if (query) {
      ...
      parts.push(
        `${t("ai.chat.message.toolParams.keywords")}: ${keywords.join(", ")}`,
      );
    }
  ...
  if (name === "get_recent_messages") {
    const parts: string[] = [];
    parts.push(
      t("ai.chat.message.toolParams.getMessages", {
  ...
  if (name === "conversation_between" || name === "get_conversation_between") {
    ...
    if (memberId1 || memberId2) {
      parts.push(
        `${t("ai.chat.message.toolParams.memberId")}: ${memberId1 ?? "?"} ↔ ${memberId2 ?? "?"}`,
      );
    }
    ...
    if (params.limit) {
      parts.push(
        t("ai.chat.message.toolParams.limit", { count: params.limit }),
      );
    }
  ...
  if (name === "message_context" || name === "get_message_context") {
    const ids =
      (params.messageIds as number[] | undefined) ||
      (params.message_ids as number[] | undefined);
    const singleId = params.messageId || params.message_id;
    const size = params.contextSize || params.context_size || 20;
    if (ids && ids.length > 0) {
      return t("ai.chat.message.toolParams.contextWithMessages", {
        msgCount: ids.length,
        contextSize: size,
      });
    }
  ...
  if (name === "member_stats" || name === "get_member_stats") {
    return t("ai.chat.message.toolParams.topMembers", {
      count: params.topN || params.top_n || 10,
    });
  }
  ...
  if (name === "time_stats" || name === "get_time_stats") {
    ...
    if (typeKey) {
      return (
        t(`ai.chat.message.toolParams.timeStats.${typeKey}`) || String(typeKey)
      );
    }
    return "";
  }
...
<template>
  ...
  >
    ...
    <div v-if="isUser" class="h-8 w-8 shrink-0 overflow-hidden rounded-full">
      <img
        :src="userAvatar"
        :alt="t('ai.chat.message.userAvatar')"
      ...
      <template v-else-if="useBlocksRendering">
        <div class="space-y-3">
          <template v-for="(block, idx) in visibleBlocks" :key="idx">
            ...
            >
              <summary
                class="cursor-pointer select-none text-xs font-medium text-gray-500 dark:text-gray-400"
              >
                ...
                >
                  ...
                  <span class="flex gap-0.5">
                    <span
                      class="h-1 w-1 animate-bounce rounded-full bg-gray-400 [animation-delay:0ms]"
                    />
                    <span
                      class="h-1 w-1 animate-bounce rounded-full bg-gray-400 [animation-delay:150ms]"
                    />
                    <span
                      class="h-1 w-1 animate-bounce rounded-full bg-gray-400 [animation-delay:300ms]"
                    />
                  </span>
                </span>
              </summary>
              <div
                class="mt-2 prose prose-sm dark:prose-invert max-w-none leading-relaxed text-xs"
              >
          ...
          <div
            v-if="
              ...
              visibleBlocks[visibleBlocks.length - 1].type === 'tool'
            "
          ...
          >
            <span class="flex gap-1">
              <span
                class="h-1.5 w-1.5 animate-bounce rounded-full bg-pink-500 [animation-delay:0ms]"
              />
              <span
                class="h-1.5 w-1.5 animate-bounce rounded-full bg-pink-500 [animation-delay:150ms]"
              />
              <span
                class="h-1.5 w-1.5 animate-bounce rounded-full bg-pink-500 [animation-delay:300ms]"
              />
            </span>
