...
const { aiPromptSettings, activePreset, aiGlobalSettings } =
...
const currentPresets = computed(() =>
  promptStore.getPresetsForChatType(props.chatType),
);

// English engineering note.
const currentActivePresetId = computed(
  () => aiPromptSettings.value.activePresetId,
);

// English engineering note.
const currentActivePreset = computed(() => {
  const activeInList = currentPresets.value.find(
    (p) => p.id === currentActivePresetId.value,
  );
...
async function openAiLogFile() {
  ...
  try {
    ...
    console.error("[ChatStatusBar] Failed to open AI log file:", error);
    toast.add({
      title: t("ai.chat.statusBar.log.openFailed"),
...
<template>
  <div class="flex items-center justify-between px-1">
    ...
    <div class="flex items-center gap-1">
      ...
      >
        <span
          >{{ t("ai.chat.statusBar.messageLimit.label")
          }}{{ aiGlobalSettings.maxMessagesPerRequest }}</span
        >
      </button>
      <!-- English UI note -->
      <button
