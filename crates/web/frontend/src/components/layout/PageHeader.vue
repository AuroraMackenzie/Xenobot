    ...
    <div class="xeno-page-header-shell flex items-center justify-between gap-4">
      <div class="xeno-page-header-leading flex items-center gap-3">
        ...
        <div v-if="avatar" class="xeno-page-header-avatar-wrap">
          <img :src="avatar" :alt="title" class="h-10 w-10 rounded-xl object-cover" />
        </div>
        <!-- English UI note -->
        <div v-else-if="icon" class="xeno-page-header-icon-wrap" :class="iconClass">
...
.xeno-page-header-atmosphere {
  ...
  pointer-events: none;
  background:
    radial-gradient(circle at 12% 10%, rgba(34, 211, 238, 0.1), transparent 28%),
    linear-gradient(90deg, rgba(255, 255, 255, 0.05), transparent 18%, transparent 82%, rgba(255, 255, 255, 0.02));
  opacity: 0.82;
}
