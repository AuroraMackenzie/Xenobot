...
/**
 * Description of a pending migration that must run before sessions are available.
 */
export interface MigrationInfo {
  version: number;
  /** Short migration title shown in the UI. */
  description: string;
  /** User-facing explanation shown before execution. */
  userMessage: string;
}

/**
 * Migration readiness summary returned by the desktop bridge.
 */
...
/** Batch import lifecycle state for a single selected file. */
...
/** Frontend record for one file participating in batch import. */
...
/** Merge import lifecycle state for an individual source file. */
export type MergeFileStatus = "pending" | "parsing" | "done";

/** Parsed metadata for one file in the merge-import flow. */
...
/** High-level merge-import stage used by the progress UI. */
export type MergeImportStage = "parsing" | "merging" | "done" | "error";

/** Final outcome of a merge-import operation. */
  ...
  () => {
    ...
    async function checkMigration(): Promise<MigrationCheckResult> {
      try {
        ...
        console.error("[Session Store] Failed to check migrations:", error);
        return {
    ...
    async function runMigration(): Promise<{
      ...
      try {
        ...
        console.error("[Session Store] Failed to run migrations:", error);
    ...
    /**
     * Diagnostics emitted by the importer for post-run troubleshooting.
     */
      ...
      try {
            ...
            if (next.stage !== currentStage) {
              ...
              if (elapsed < MIN_STAGE_TIME) {
                await new Promise((resolve) =>
                  setTimeout(resolve, MIN_STAGE_TIME - elapsed),
                );
              }
        ...
        if (importResult.success && importResult.sessionId) {
          ...
          try {
            ...
            const gapThreshold = savedThreshold
              ? parseInt(savedThreshold, 10)
              : 1800; // English engineering note.
            await window.sessionApi.generate(
              importResult.sessionId,
              gapThreshold,
            );
          } catch (error) {
            console.error(
              "[Session Store] Failed to auto-generate the session index:",
              error,
            );
            // English engineering note.
          }
     ...
     * English note.
     */
    async function importFilesFromPaths(
      filePaths: string[],
     ...
     * English note.
     */
    async function mergeImportFiles(
      filePaths: string[],
    ): Promise<MergeImportResult> {
      if (filePaths.length < 2) {
        return {
          success: false,
          error: "Merged import requires at least two files.",
        };
      }
      ...
      try {
        ...
        for (let i = 0; i < mergeFiles.value.length; i++) {
          ...
          try {
            ...
            const minFileTime = Math.max(
              300,
              MIN_STAGE_TIME / filePaths.length,
            );
            if (elapsed < minFileTime) {
              await new Promise((resolve) =>
                setTimeout(resolve, minFileTime - elapsed),
              );
            }
            ...
            throw new Error(
              `Failed to parse file: ${file.name} - ${err instanceof Error ? err.message : String(err)}`,
            );
          }
        }
        ...
        if (parsingElapsed < MIN_STAGE_TIME) {
          await new Promise((resolve) =>
            setTimeout(resolve, MIN_STAGE_TIME - parsingElapsed),
          );
        }
        ...
        if (!result.success) {
          throw new Error(result.error || "Merged import failed.");
        }
        ...
        if (mergingElapsed < MIN_STAGE_TIME) {
          await new Promise((resolve) =>
            setTimeout(resolve, MIN_STAGE_TIME - mergingElapsed),
          );
        }
    ...
    async function deleteSession(id: string): Promise<boolean> {
      try {
        ...
        console.error("[Session Store] Failed to delete the session:", error);
        return false;
      }
    }
    ...
    async function renameSession(
      ...
      try {
        ...
        console.error("[Session Store] Failed to rename the session:", error);
        return false;
      }
    }
     ...
     * English note.
     */
    async function updateSessionOwnerId(
      id: string,
      ownerId: string | null,
    ): Promise<boolean> {
      try {
        ...
        console.error(
          "[Session Store] Failed to update the session owner:",
          error,
        );
        return false;
      }
    ...
    const sortedSessions = computed(() => {
      ...
      const pinIndexMap = new Map(
        pinnedSessionIds.value.map((id, index) => [id, index]),
      );
