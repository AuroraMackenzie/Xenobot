...
const isPartialSelected = computed(() => {
  const selectedInFiltered = filteredSessions.value.filter((s) =>
    selectedIds.value.has(s.id),
  ).length;
  return (
    selectedInFiltered > 0 && selectedInFiltered < filteredSessions.value.length
  );
});
...
const PLATFORM_CONFIG: Record<string, { label: string; class: string }> = {
  ...
  line: {
    label: "LINE",
    class: "bg-lime-100 text-lime-700 dark:bg-lime-900/30 dark:text-lime-300",
  },
  unknown: {
    label: "未知",
...
async function executeMerge() {
  ...
  try {
    // English engineering note.
    const exportResult =
      await window.chatApi.exportSessionsToTempFiles(selectedSessionIds);
    ...
    if (conflictResult.conflicts.length > 0) {
      ...
      // English engineering note.
      console.log(
        `[BatchDelete] 检测到 ${conflictResult.conflicts.length} 个冲突，使用默认解决方案`,
      );
    }

    // English engineering note.
    mergeProgress.value = t("tools.batchManage.mergeSteps.merging");
    const firstSession = sessions.value.find((s) =>
      selectedIds.value.has(s.id),
    );
    ...
    // English engineering note.
    toast.add({
      title: t("tools.batchManage.mergeSuccess", {
  ...
  <div class="xeno-batch-manage flex h-full flex-col">
    ...
    <div class="xeno-batch-toolbar mb-4 flex items-center justify-between">
      <div class="xeno-batch-toolbar-meta flex items-center gap-4">
        ...
        >
          {{
            t("tools.batchManage.searchResult", {
    ...
    <div
      ...
      class="xeno-batch-empty flex flex-1 items-center justify-center"
    >
      ...
      >
        ...
        <div class="w-6">
          <UCheckbox
            ...
            @click.stop="handleRowClick(index, session.id, $event)"
          />
        </div>
      ...
      <template #content>
        <div class="xeno-batch-modal-shell p-4">
          ...
          <p class="mb-4 text-gray-600 dark:text-gray-400">
            {{
              t("tools.batchManage.mergeConfirmMessage", {
          ...
          <!-- English UI note -->
          <div
            class="xeno-batch-modal-list mb-4 max-h-40 overflow-y-auto rounded-lg border border-gray-200 dark:border-gray-700"
          >
            <div
            ...
            >
              ...
              <span class="text-xs text-gray-400"
                >{{ session.messageCount.toLocaleString() }} 条</span
              >
            </div>
          </div>
          ...
          <div
            v-if="isMerging"
            class="mb-4 rounded-lg bg-blue-50 px-4 py-3 dark:bg-blue-900/20"
          >
