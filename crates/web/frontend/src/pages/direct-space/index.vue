...
const tabs = [
  ...
  {
    id: "quotes",
    labelKey: "analysis.tabs.quotes",
    icon: "i-heroicons-chat-bubble-left-right",
  },
  {
    id: "member",
    labelKey: "analysis.tabs.member",
...
async function loadBaseData() {
  ...
  try {
    ...
    console.error("[DirectSpace] Failed to load base data:", error);
  }
}
...
async function loadAnalysisData() {
  ...
  try {
    ...
    console.error("[DirectSpace] Failed to load analysis data:", error);
  } finally {
...
const otherMemberAvatar = computed(() => {
  ...
  if (session.value.ownerId) {
    const other = memberActivity.value.find(
      (m) => m.platformId !== session.value?.ownerId,
    );
    if (other?.avatar) return other.avatar;
  }

  // English engineering note.
  const sameName = memberActivity.value.find(
    (m) => m.name === session.value?.name,
  );
...
<template>
  ...
  >
    ...
    <LoadingState
      v-if="isInitialLoad"
      variant="page"
      :text="t('analysis.privateChat.loading')"
    />
