...
async function loadMembers() {
  ...
  try {
    ...
    console.error(
      "[CircleSpaceMemberList] Failed to load paginated members:",
      error,
    );
  } finally {
...
<template>
  <div class="main-content max-w-5xl p-6">
        ...
        <div class="max-h-[500px] overflow-auto">
          <table class="min-w-[860px] w-full">
            <thead class="sticky top-0 bg-slate-950/90 backdrop-blur-md">
              <tr
                class="text-left text-xs font-medium uppercase text-gray-500 dark:text-gray-400"
              >
                <th class="px-4 py-4 min-w-[240px]">
                  {{ t("members.list.table.accountName") }}
                </th>
    ...
    <!-- English UI note -->
    <div
      class="xeno-member-warning mt-4 flex items-start gap-3 rounded-2xl p-4"
    >
      <UIcon
