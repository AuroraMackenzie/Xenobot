...
async function loadMembersWithNicknameChanges() {
  ...
  try {
    ...
    console.error(
      "[NicknameHistory] Failed to load nickname change history:",
      error,
    );
  } finally {
  ...
  <div class="xeno-nickname-shell main-content max-w-5xl p-6">
      ...
      :description="
        isLoadingHistory
          ...
          : membersWithNicknameChanges.length > 0
            ? t('members.nicknameHistory.hasChanges', {
                count: membersWithNicknameChanges.length,
              })
    ...
    >
      ...
      >
        ...
        >
          <div
            class="w-32 shrink-0 pt-0.5 font-medium text-gray-900 dark:text-white"
          >
            {{ member.name }}
          </div>
          ...
          <div class="flex flex-1 flex-wrap items-center gap-2">
            <template v-for="(item, index) in member.history" :key="index">
              <div
                class="xeno-nickname-chip flex items-center gap-1.5 rounded-xl px-3 py-1.5"
              >
                <span
                  class="text-sm"
                  :class="
                ...
                <span class="text-xs text-gray-400"
                  >({{ formatPeriod(item.startTs, item.endTs) }})</span
                >
              </div>
      ...
      <EmptyState
        ...
        :text="t('members.nicknameHistory.empty')"
      />
