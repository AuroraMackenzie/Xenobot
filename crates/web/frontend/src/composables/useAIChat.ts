...
export function useAIChat(
  ...
  const agentToolAliasToCanonical = ref<Record<string, string>>(
    buildFallbackAliasMap(),
  );
  ...
  // English engineering note.
  const sessionTokenUsage = ref<TokenUsage>({
    promptTokens: 0,
    completionTokens: 0,
    totalTokens: 0,
  });
    ...
    try {
      ...
      if (ownerMember) {
        ownerInfo.value = {
          platformId: ownerId,
          displayName:
            ownerMember.groupNickname || ownerMember.accountName || ownerId,
        };
  ...
  async function sendMessage(content: string): Promise<void> {
    ...
    if (!content.trim() || isAIThinking.value) {
      console.log(
        "[AI] Skipped because content is empty or the assistant is already thinking",
      );
      return;
    }
    ...
    if (!hasConfig) {
      ...
      messages.value.push({
        ...
        role: "assistant",
        content:
          "⚠️ 请先配置 AI 服务。点击左下角「设置」按钮前往「模型配置Tab」进行配置。",
        timestamp: Date.now(),
      });
      return;
    }
    ...
    console.log("[AI] Starting agent processing...", {
    ...
    try {
      ...
      const context = {
        ...
        timeFilter: timeFilter
          ? { startTs: timeFilter.startTs, endTs: timeFilter.endTs }
          : undefined,
        maxMessagesLimit: aiGlobalSettings.value.maxMessagesPerRequest,
        ownerInfo: ownerInfo.value
          ? {
              platformId: ownerInfo.value.platformId,
              displayName: ownerInfo.value.displayName,
            }
          : undefined,
      };
      ...
      // English engineering note.
      const { requestId: agentReqId, promise: agentPromise } =
        window.agentApi.runStream(
          content,
          context,
          (chunk) => {
            // English engineering note.
            if (isAborted || thisRequestId !== currentRequestId) {
              console.log(
                "[AI] Ignoring chunk because the request was aborted or expired",
                {
                  isAborted,
                  thisRequestId,
                  currentRequestId,
            ...
            switch (chunk.type) {
              ...
              case "tool_start":
                ...
                if (chunk.toolName) {
                  const normalizedToolName = normalizeAgentToolName(
                    chunk.toolName,
                  );
                  if (!normalizedToolName) {
                    break;
                  }
                  const toolParams = chunk.toolParams as
              ...
              case "done":
                ...
                if (chunk.usage) {
                  sessionTokenUsage.value = {
                    promptTokens:
                      sessionTokenUsage.value.promptTokens +
                      chunk.usage.promptTokens,
                    completionTokens:
                      sessionTokenUsage.value.completionTokens +
                      chunk.usage.completionTokens,
                    totalTokens:
                      sessionTokenUsage.value.totalTokens +
                      chunk.usage.totalTokens,
                  };
                  console.log(
                    "[AI] Token usage updated:",
                    sessionTokenUsage.value,
                  );
                }
                break;
              ...
              case "error":
                ...
                if (!hasStreamError) {
                  ...
                  const errorMessage =
                    chunk.errorMessage || chunk.error || "Unknown error";
                  // English engineering note.
                  appendTextToBlocks(
      ...
      if (result.success && result.result) {
        ...
        const detail =
          ...
          "Unknown error";
        const errorCode =
          (result as { error?: string; errorMessage?: string }).error ||
          "error.agent_runtime";
...
- model configuration`,
  ...
  async function saveConversation(
    ...
    try {
      ...
      if (!currentConversationId.value) {
        ...
        const conversation = await window.aiApi.createConversation(
          sessionId,
          title,
        );
  ...
  async function loadConversation(conversationId: string): Promise<void> {
    console.log(
      "[AI] Loading conversation history, conversationId:",
      conversationId,
    );
    try {
      ...
      console.log(
        "[AI] Conversation history loaded, messages count:",
