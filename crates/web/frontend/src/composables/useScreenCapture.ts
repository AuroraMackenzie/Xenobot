...
export function useScreenCapture() {
  ...
  async function captureElement(
    ...
    if (options?.mobileWidth) {
      const baseWidth =
        typeof options.mobileWidth === "number"
          ? options.mobileWidth
      ...
      if (currentWidth > baseWidth) {
        ...
        const targetWidth = Math.round(
          baseWidth + (currentWidth - baseWidth) * scaleFactor,
        );
    ...
    lists.forEach((list) => {
      ...
      lis.forEach((li, index) => {
        ...
        prefix.style.cssText =
          "display: inline-block; min-width: 1.5em; margin-right: 0.25em; text-align: right;";
