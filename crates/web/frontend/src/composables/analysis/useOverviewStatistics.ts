...
export function useOverviewStatistics(
  props: UseOverviewStatisticsProps,
  weekdayActivity: Ref<WeekdayActivity[]>,
) {
  const { t } = useI18n();
  ...
  const durationDays = computed(() => {
    if (props.selectedYear) {
      const isLeapYear =
        (props.selectedYear % 4 === 0 && props.selectedYear % 100 !== 0) ||
        props.selectedYear % 400 === 0;
  ...
  const displayMemberCount = computed(() => {
    return props.selectedYear
      ? props.filteredMemberCount
      : props.session.memberCount;
  });
  ...
  const peakDay = computed(() => {
    ...
    return props.dailyActivity.reduce(
      (max, d) => (d.messageCount > max.messageCount ? d : max),
      props.dailyActivity[0],
    );
  });
  ...
  const activeRate = computed(() => {
    return totalDays.value > 0
      ? Math.round((activeDays.value / totalDays.value) * 100)
      : 0;
  });
