...
pub fn transcode_audio_to_mp3(
    ...
    if !input_path.exists() {
        return Err(ZoomError::Io(std::io::Error::new(
    ...
    if let Some(parent) = output_path.parent() {
        std::fs::create_dir_all(parent).map_err(ZoomError::Io)?;
    }
    ...
    let output = Command::new(binary)
        ...
        .output()
        .map_err(|error| ZoomError::Internal(anyhow::anyhow!("failed to start ffmpeg: {}", error)))?;
    ...
    Err(ZoomError::Internal(anyhow::anyhow!(
...
pub fn transcode_audio_bytes_to_mp3(
    ...
    if input_bytes.is_empty() {
        return Err(ZoomError::Internal(anyhow::anyhow!("input audio payload is empty")));
    }
    ...
    if let Some(mut stdin) = child.stdin.take() {
        stdin.write_all(input_bytes).map_err(|error| {
            ZoomError::Internal(anyhow::anyhow!("failed to write ffmpeg stdin: {}", error))
        })?;
    }

    let output = child.wait_with_output().map_err(|error| {
        ZoomError::Internal(anyhow::anyhow!("failed to wait ffmpeg: {}", error))
    })?;

    if output.status.success() {
        if output.stdout.is_empty() {
            return Err(ZoomError::Internal(anyhow::anyhow!(
...
mod tests {
    ...
    fn missing_input_file_returns_not_found() {
        let options = AudioTranscodeOptions::default();
        let input_path = Path::new("/tmp/zoom-missing-input.ogg");
        let output_path = Path::new("/tmp/zoom-missing-output.mp3");
        let error = transcode_audio_to_mp3(input_path, output_path, &options)
            .expect_err("missing input should fail");

        match error {
            ZoomError::Io(inner) => assert_eq!(inner.kind(), std::io::ErrorKind::NotFound),
