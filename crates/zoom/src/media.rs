//! Media helpers for legal-safe Zoom export workflows.

use std::fs;
use std::path::{Path, PathBuf};

/// Media kind inferred from a Zoom export asset path.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ZoomMediaKind {
    /// Image asset.
    Image,
    /// Video asset.
    Video,
...
pub struct ZoomMediaAsset {
    /// Original asset path.
    pub path: PathBuf,
    /// Inferred media kind.
    pub kind: ZoomMediaKind,
...
pub fn classify_media_path(path: &Path) -> ZoomMediaKind {
    let ext = path
        .extension()
    ...
    match ext.as_deref() {
        Some("jpg" | "jpeg" | "png" | "gif" | "bmp" | "heic" | "webp") => ZoomMediaKind::Image,
        Some("mp4" | "mov" | "avi" | "mkv" | "webm") => ZoomMediaKind::Video,
        Some("ogg" | "opus" | "mp3" | "m4a" | "wav" | "aac") => ZoomMediaKind::Audio,
        Some("pdf" | "doc" | "docx" | "xls" | "xlsx" | "ppt" | "pptx" | "txt" | "csv") => {
            ZoomMediaKind::Document
        }
        Some("zip" | "7z" | "rar" | "tar" | "gz") => ZoomMediaKind::Archive,
        Some("svg") => ZoomMediaKind::Sticker,
        _ => ZoomMediaKind::Unknown,
    }
}

/// Build a media inventory from explicitly provided asset paths.
pub fn collect_media_assets<I, P>(paths: I) -> Vec<ZoomMediaAsset>
where
...
mod tests {
    ...
    fn collects_existing_media_assets() {
        ...
        assert_eq!(assets[0].kind, ZoomMediaKind::Image);
        assert_eq!(assets[0].size_bytes, 3);
    }

    #[test]
    fn classifies_unknown_extensions_as_unknown() {
        assert_eq!(classify_media_path(Path::new("mystery.bin")), ZoomMediaKind::Unknown);
    }
