//! Media helpers for legal-safe Skype export workflows.

use std::fs;
use std::path::{Path, PathBuf};

/// Media kind inferred from a Skype export asset path.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SkypeMediaKind {
    /// Image asset.
    Image,
    /// Video asset.
    Video,
...
pub struct SkypeMediaAsset {
    /// Original asset path.
    pub path: PathBuf,
    /// Inferred media kind.
    pub kind: SkypeMediaKind,
...
pub fn classify_media_path(path: &Path) -> SkypeMediaKind {
    let ext = path
        .extension()
    ...
    match ext.as_deref() {
        Some("jpg" | "jpeg" | "png" | "gif" | "bmp" | "heic" | "webp") => SkypeMediaKind::Image,
        Some("mp4" | "mov" | "avi" | "mkv" | "webm") => SkypeMediaKind::Video,
        Some("ogg" | "opus" | "mp3" | "m4a" | "wav" | "aac") => SkypeMediaKind::Audio,
        Some("pdf" | "doc" | "docx" | "xls" | "xlsx" | "ppt" | "pptx" | "txt" | "csv") => {
            SkypeMediaKind::Document
        }
        Some("zip" | "7z" | "rar" | "tar" | "gz") => SkypeMediaKind::Archive,
        Some("svg") => SkypeMediaKind::Sticker,
        _ => SkypeMediaKind::Unknown,
    }
}

/// Build a media inventory from explicitly provided asset paths.
pub fn collect_media_assets<I, P>(paths: I) -> Vec<SkypeMediaAsset>
where
...
mod tests {
    ...
    fn collects_existing_media_assets() {
        ...
        assert_eq!(assets[0].kind, SkypeMediaKind::Image);
        assert_eq!(assets[0].size_bytes, 3);
    }

    #[test]
    fn classifies_unknown_extensions_as_unknown() {
        assert_eq!(classify_media_path(Path::new("mystery.bin")), SkypeMediaKind::Unknown);
    }
