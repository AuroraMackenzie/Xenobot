//! Account models for legal-safe Skype contexts.
//!
...
/// Represents a discovered Skype account context.
...
mod tests {
    ...
    fn builds_account_from_source() {
        let candidate = source(
            ...
            "macOS Skype sandbox data",
            "/Users/test/Library/Containers/Skype",
        );
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "macOS Skype sandbox data");
    ...
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/skype-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "skype-export");
    }

    #[test]
    fn root_path_reflects_data_dir() {
        let candidate = source(SourceKind::ExportDirectory, "Archive", "/tmp/skype-root");
        let account = Account::from_source(&candidate);

        assert_eq!(account.root_path(), Path::new("/tmp/skype-root"));
    }
}
