//! Account models for legal-safe Teams contexts.
//!
...
/// Represents a discovered Teams account context.
...
mod tests {
    ...
    fn builds_account_from_source() {
        let candidate = source(
            ...
            "macOS Teams sandbox data",
            "/Users/test/Library/Containers/Teams",
        );
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "macOS Teams sandbox data");
    ...
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/teams-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "teams-export");
    }

    #[test]
    fn root_path_reflects_data_dir() {
        let candidate = source(SourceKind::ExportDirectory, "Archive", "/tmp/teams-root");
        let account = Account::from_source(&candidate);

        assert_eq!(account.root_path(), Path::new("/tmp/teams-root"));
    }
