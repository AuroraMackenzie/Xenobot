...
use crate::{SlackError, SlackResult};
use std::io::Write;
...
pub fn transcode_audio_to_mp3(
    ...
    if !input_path.exists() {
        return Err(SlackError::Io(std::io::Error::new(
    ...
    let output = Command::new(binary)
        ...
        .output()
        .map_err(|error| SlackError::Internal(anyhow::anyhow!("failed to start ffmpeg: {}", error)))?;
    ...
    Err(SlackError::Internal(anyhow::anyhow!(
...
pub fn transcode_audio_bytes_to_mp3(
    ...
    if input_bytes.is_empty() {
        return Err(SlackError::Internal(anyhow::anyhow!("input audio payload is empty")));
    }
    ...
    if let Some(mut stdin) = child.stdin.take() {
        stdin.write_all(input_bytes).map_err(|error| {
            SlackError::Internal(anyhow::anyhow!("failed to write ffmpeg stdin: {}", error))
        })?;
    }

    let output = child.wait_with_output().map_err(|error| {
        SlackError::Internal(anyhow::anyhow!("failed to wait ffmpeg: {}", error))
    })?;

    if output.status.success() {
        if output.stdout.is_empty() {
            return Err(SlackError::Internal(anyhow::anyhow!(
...
mod tests {
    ...
    fn missing_input_file_returns_not_found() {
        let options = AudioTranscodeOptions::default();
        let input_path = Path::new("/tmp/slack-missing-input.ogg");
        let output_path = Path::new("/tmp/slack-missing-output.mp3");
        let error = transcode_audio_to_mp3(input_path, output_path, &options)
            .expect_err("missing input should fail");

        match error {
            SlackError::Io(inner) => assert_eq!(inner.kind(), std::io::ErrorKind::NotFound),
