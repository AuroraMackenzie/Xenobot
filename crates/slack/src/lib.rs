//! Legal-safe Slack adapter for Xenobot.
//!
...
pub use service::{AuthorizedSlackWorkspace, SlackService, StagedSlackExport};
/// Common result type used by Slack crate operations.
pub type SlackResult<T> = Result<T, SlackError>;

/// Stable platform identifier.
pub const PLATFORM_ID: &str = "slack";

/// Legal-safe adapter for Slack workflows.
...
impl SlackAdapter {
    ...
    pub fn parse_authorized_export(&self, path: &Path) -> Result<ParsedChat, SlackError> {
        ...
        let parsed = registry.detect_and_parse(path).map_err(SlackError::Parse)?;
        if parsed.platform.eq_ignore_ascii_case(PLATFORM_ID) {
            Ok(parsed)
        } else {
            Err(SlackError::PlatformMismatch {
...
/// Errors returned by the Slack adapter.
#[derive(Debug, Error)]
pub enum SlackError {
