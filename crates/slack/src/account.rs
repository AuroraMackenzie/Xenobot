//! Account models for legal-safe Slack contexts.
//!
...
/// Represents a discovered Slack account context.
...
mod tests {
    ...
    fn builds_account_from_source() {
        let candidate = source(
            ...
            "macOS Slack sandbox data",
            "/Users/test/Library/Containers/Slack",
        );
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "macOS Slack sandbox data");
    ...
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/slack-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "slack-export");
    }

    #[test]
    fn root_path_reflects_data_dir() {
        let candidate = source(SourceKind::ExportDirectory, "Archive", "/tmp/slack-root");
        let account = Account::from_source(&candidate);
