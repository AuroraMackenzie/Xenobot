//! Service layer for legal-safe Instagram ingestion workflows.
...
use crate::media::{collect_media_assets, InstagramMediaAsset};
use crate::monitor::{FileMonitor, FileMonitorConfig};
use crate::{InstagramAdapter, InstagramError};

/// Parsed export staged by the Instagram service.
#[derive(Debug, Clone)]
pub struct StagedInstagramExport {
...
/// Aggregated legal-safe workspace assembled from explicit Instagram inputs.
#[derive(Debug, Clone)]
pub struct AuthorizedInstagramWorkspace {
    /// Stable platform identifier.
    pub platform_id: &'static str,
...
/// Legal-safe Instagram orchestration service.
...
impl InstagramService {
    ...
    pub fn stage_authorized_exports<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<StagedInstagramExport>, InstagramError>
    where
    ...
    pub fn collect_media_inventory<I, P>(
        &self,
        paths: I,
    ) -> Result<Vec<InstagramMediaAsset>, InstagramError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
    {
        let resolved: Result<Vec<PathBuf>, InstagramError> = paths
            .into_iter()
            .map(|path| {
    ...
    pub fn build_authorized_workspace<I, P, J, Q>(
        ...
        export_paths: I,
        media_paths: J,
    ) -> Result<AuthorizedInstagramWorkspace, InstagramError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        ...
        Q: AsRef<Path>,
    {
        ...
        Ok(AuthorizedInstagramWorkspace {
            platform_id: self.platform_id(),
            accounts,
            primary_account,
            staged_exports,
    ...
    ) -> Result<(AuthorizedInstagramWorkspace, Option<FileMonitor>), InstagramError>
    where
        I: IntoIterator<Item = P>,
        P: AsRef<Path>,
        J: IntoIterator<Item = Q>,
    ...
    pub fn create_export_monitor(&self, watch_dir: &Path) -> Result<FileMonitor, InstagramError> {
        self.ensure_authorized(watch_dir)?;
        FileMonitor::new(FileMonitorConfig {
            watch_dir: watch_dir.to_path_buf(),
            file_patterns: FileMonitor::instagram_export_patterns(),
            recursive: true,
        })
    }

    fn ensure_authorized(&self, path: &Path) -> Result<(), InstagramError> {
...
mod tests {
    ...
    fn returns_platform_id_from_service() {
        let service = InstagramService::new(InstagramConfig::default());
        assert_eq!(service.platform_id(), "instagram");
    }
    ...
    fn rejects_paths_outside_authorized_roots() {
        ...
        match err {
            InstagramError::UnauthorizedPath { path } => {
    ...
    fn build_authorized_workspace_rejects_unauthorized_media_paths() {
        ...
        let export = export_dir.path().join("instagram_fixture.json");
    ...
    fn builds_authorized_workspace_from_exports_and_media() {
        ...
        assert_eq!(workspace.platform_id, "instagram");
    ...
    fn prepare_authorized_workspace_rejects_unauthorized_watch_directory() {
        ...
        let export = input_dir.path().join("instagram_fixture.json");
        write_fixture(
            &export,
            r#"[{"sender":"Alice","timestamp":1735813230,"content":"hello instagram"}]"#,
        );
    ...
    fn collect_media_inventory_rejects_unauthorized_assets() {
        ...
        match error {
            InstagramError::UnauthorizedPath { path } => assert_eq!(path, asset),
            other => panic!("unexpected error: {other:?}"),
        }
    }
