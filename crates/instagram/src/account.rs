//! Account models for legal-safe Instagram contexts.
//!
...
/// Represents a discovered Instagram account context.
...
mod tests {
    ...
    fn builds_account_from_source() {
        let candidate = source(
            ...
            "macOS Instagram sandbox data",
            "/Users/test/Library/Containers/Instagram",
        );
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "macOS Instagram sandbox data");
    ...
    fn falls_back_to_directory_name_when_label_is_blank() {
        let candidate = source(SourceKind::ExportDirectory, "   ", "/tmp/instagram-export");
        let account = Account::from_source(&candidate);

        assert_eq!(account.name, "instagram-export");
    }

    #[test]
    fn root_path_reflects_data_dir() {
        let candidate = source(SourceKind::ExportDirectory, "Archive", "/tmp/instagram-root");
        let account = Account::from_source(&candidate);

        assert_eq!(account.root_path(), Path::new("/tmp/instagram-root"));
    }
